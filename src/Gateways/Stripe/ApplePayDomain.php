 ...
 * admin: they are not secret, but they do change, so Gratora stores whatever the
...
final class ApplePayDomain extends HookProvider
{
    ...
    public function refresh(bool $test): array
    {
        ...
        if (! is_array($first)) {
            return ['status' => 'unknown', 'message' => __('This domain is not registered with Stripe yet.', 'gratora-donation-platform')];
        }
