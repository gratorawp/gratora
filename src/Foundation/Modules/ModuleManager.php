...
/**
 * Registry and boot orchestrator for Gratora modules.
 *
 ...
 * is not satisfied by the running GRATORA_VERSION.
 *
 * @since 1.0.0
 */
final class ModuleManager
{
    /** @var array<string, GratoraModule> */
     ...
     * met: id => [running GRATORA_VERSION, declared constraint].
     *
    ...
    /** @since 1.0.0 */
    public function register(GratoraModule $module): void
    {
        $id = $module->id();

        if (isset($this->modules[$id])) {
            throw new RuntimeException(esc_html("Gratora module '{$id}' is already registered."));
        }
    ...
    /** @since 1.0.0 */
    public function get(string $id): ?GratoraModule
    {
     ...
     *
     * @return array<string, array{0:string,1:string}> id => [GRATORA_VERSION, constraint]
     * @since 1.0.0
     */
    ...
    private function bootModule(string $id): void
    {
        ...
        if (is_string($coreConstraint) && $coreConstraint !== ''
            && ! VersionConstraint::satisfies(GRATORA_VERSION, $coreConstraint)
        ) {
            $this->incompatible[$id] = [GRATORA_VERSION, $coreConstraint];
            $this->booted[$id] = false;
            do_action('gratora.module.incompatible', $id, GRATORA_VERSION, $coreConstraint);
            return;
        }
