...
final class InvalidReferenceToken extends RuntimeException
{
    ...
    public function __construct(public readonly string $field, public readonly string $value)
    {
        parent::__construct(sprintf(
            ...
            __('%1$s can only use letters, numbers, hyphens and underscores, and cannot be empty. "%2$s" would be stripped down before a single reference was minted, so the numbering an accountant is given would not be the one on screen.', 'dono-fundraising-platform'),
            $field,
            $value
        ));
    }
