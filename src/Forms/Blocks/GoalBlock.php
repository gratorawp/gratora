...
final class GoalBlock implements Block
{
    ...
    private function renderCampaign(array $attrs): string
    {
        ...
        if (! $campaign) {
            return $this->missing(__('Goal will appear once the form is linked to a campaign.', 'gratora-donation-platform'));
        }
        ...
        if ($target <= 0) {
            return $this->missing(__('The campaign has no goal set yet.', 'gratora-donation-platform'));
        }
        $current = match ($type) {
    ...
    private function renderForm(array $attrs): string
    {
        ...
        if (! $form) {
            return $this->missing(__('Goal will appear once the form is published.', 'gratora-donation-platform'));
        }
        ...
        if (! in_array($type, ['amount', 'donations', 'donors'], true)) {
            return $this->missing(__('This form has no goal set. Add one in the form settings.', 'gratora-donation-platform'));
        }
        $target = $type === 'amount'
