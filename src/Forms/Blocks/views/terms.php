...
<div class="fundkit-block fundkit-block--terms fundkit-terms">
    <label class="fundkit-terms__agree">
        <input type="checkbox"
               ...
               value="1"
               required>
        <span class="fundkit-terms__label">
            <?php echo esc_html($labelText); ?>
            <span class="fundkit-terms__required" aria-hidden="true">*</span>
        </span>
    </label>

    <?php if (trim($terms) !== ''): ?>
        <?php // Scrolls rather than grows: long terms would push the submit button off the screen. ?>
        <div class="fundkit-terms__text" tabindex="0" role="region" aria-label="<?php echo esc_attr($labelText); ?>">
            <?php echo wp_kses_post(wpautop($terms)); ?>
        </div>
    <?php endif; ?>

    <?php if (trim($linkUrl) !== ''): ?>
        <p class="fundkit-terms__link">
