...
final class DonorService
{
     ...
     */
    /**
     * Take a donor off the working list.
     *
     * Nothing about the person changes: no mandate is cancelled, no mail is
     * suppressed and the portal keeps working. A donor row carries no gateway
     * handle, so this is a visibility verb and nothing more.
     *
     * Offered exactly where the delete is offered, because the bin stages that
     * delete. A donor whose money is on the books is what Redact is for.
     *
     * @throws InvalidArgumentException When the delete gate keeps them.
     *
     * @since 1.0.0
     */
    public function trash(Donor $donor): void
    {
        ...
        $donor->updateColumns([
            'trashed_at' => $now,
            // 0 is not a user id. A sweep or a CLI run has nobody behind it,
            // and the column is read to render a name.
            'trashed_by' => $by > 0 ? $by : null,
            'updated_at' => $now,
        ]);

        $this->events()->record('donor.trashed', [
            'donor_id' => (int) $donor->id,
            'payload'  => ['by' => self::actorKind(), 'actor_name' => self::actorName()],
        ]);
    }

    /**
     * Put them back. Reversible in full: the trash wrote nothing else.
     *
     * @since 1.0.0
     */
    public function restore(Donor $donor): void
    {
        if ($donor->trashed_at === null) {
            return;
        }
        ...
        $this->events()->record('donor.restored', [
            'donor_id' => (int) $donor->id,
            'payload'  => ['by' => self::actorKind(), 'actor_name' => self::actorName()],
        ]);
    }
