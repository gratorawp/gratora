...
final class DonorRepository
{
    ...
    {
        $prefix = DB::getPrefix();
        $any    = "SELECT 1 FROM {$prefix}fundkit_donations d WHERE d.donor_id = {$prefix}fundkit_donors.id";
    ...
    private function givingDonorPredicate(): string
    {
        ...
        return "({$prefix}fundkit_donors.donations_count > 0 OR EXISTS (SELECT 1 FROM {$prefix}fundkit_donations d "
            . "WHERE d.donor_id = {$prefix}fundkit_donors.id AND d.is_test = 0))";
    }
    ...
    public function aggregateAdmin(array $args = []): array
    {
        ...
        $base = fn () => DB::table('fundkit_donors');
    ...
    public function listAtRisk(string $today, int $page = 1, int $perPage = 25, int $activeDays = self::ACTIVE_DAYS, int $atRiskDays = self::AT_RISK_DAYS): array
    {
        ...
        $total = (int) DB::table('fundkit_donors')->whereRaw($where)->count();
    ...
    {
        $donorsT    = DB::getPrefix() . 'fundkit_donors';
        $donationsT = DB::getPrefix() . 'fundkit_donations';
