...
<p class="raised"><?php echo esc_html(sprintf(/* translators: %s: formatted amount raised */ __('%s raised', 'fundraising-toolkit'), $raised)); ?></p>
<?php if ($has_goal): ?>
    <p class="goal"><?php echo esc_html(sprintf(
        /* translators: 1: formatted goal, 2: percent complete */
        __('of %1$s goal (%2$d%% complete)', 'fundraising-toolkit'),
        $goal_display,
        $percent
    )); ?></p>
    <div class="bar"><span style="width:<?php echo (int) $bar_width; ?>%"></span></div>
<?php else: ?>
    <p class="goal"><?php esc_html_e('No goal set for this campaign.', 'fundraising-toolkit'); ?></p>
<?php endif; ?>
...
<p class="footer"><?php echo esc_html(sprintf(
    ...
    __('Generated %1$s, %2$s', 'fundraising-toolkit'),
    $generated_date,
    $org_name
)); ?></p>
</body></html>
