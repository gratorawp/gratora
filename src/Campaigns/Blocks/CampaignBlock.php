...
abstract class CampaignBlock implements Block
{
     ...
     * put the same tokens on the body, where an organizer's own headings and
     ...
     * ordinary page, so its blocks answer to the editor's own color, spacing
    ...
    protected function notBoundNotice(array $attrs = []): string
    {
        ...
        $message = $bound === null
            ? __('This block is not bound to a campaign. Pick one in the block sidebar.', 'gratora-donation-platform')
            : sprintf(
                /* translators: %s: the campaign's status, e.g. "draft". */
                __('This campaign is %s, so this block is hidden from visitors. Publish the campaign to show it.', 'gratora-donation-platform'),
                (string) $bound->status
            );
