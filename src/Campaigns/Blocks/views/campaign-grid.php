...
echo wp_kses_data(get_block_wrapper_attributes(array_filter([
    'class' => 'gratora-block gratora-block--grid',
    'style' => $styleVars,
])));
?> data-block="gratora/campaign-grid">
    <?php if ($heading !== ''): ?>
        <div class="gratora-grid__head">
            <h2 class="gratora-grid__title"><?php echo esc_html($heading);
?></h2>
        </div>
    <?php endif; ?>
    <?php if ($cards === []): ?>
...
?></div>
        <?php endif; ?>
    <?php else: ?>
    <div class="gratora-campaign-grid">
...
?>" loading="lazy" />
                ...
                <span class="gratora-campaign-card__body">
                    <span class="gratora-campaign-card__title"><?php echo esc_html($card['title']);
?></span>
                    <?php if ($card['blurb'] !== ''): ?>
                        <span class="gratora-campaign-card__blurb"><?php echo esc_html(wp_strip_all_tags($card['blurb']));
?></span>
                    <?php endif; ?>
...
?></span>
                            <?php if ($card['goalLabel'] !== ''): ?>
                                <span class="gratora-campaign-card__goal"><?php echo esc_html($card['goalLabel']);
?></span>
                            <?php endif; ?>
                        </span>
                    </span>
                    <span class="gratora-campaign-card__foot">
                        <?php if ($card['percent'] > 0): ?>
                            <span class="gratora-campaign-card__pct"><?php echo (int) $card['percent']; ?>%</span>
                        <?php else: ?>
                            <span></span>
                        <?php endif; ?>
                        <span class="gratora-campaign-card__link">
                            <?php esc_html_e('Donate', 'gratora-donation-platform');
?>
