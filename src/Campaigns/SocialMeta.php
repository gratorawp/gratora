 ...
 * the tag set per route via the gratora.social_meta filter (P2P swaps in the
 * fundraiser's or team's own meta). A detected SEO plugin owns social meta,
 * so we stand down unless gratora.social_meta.enabled opts back in.
 *
 * @since 1.0.0
 */
final class SocialMeta extends HookProvider
{
    ...
    public function printTags(): void
    {
        ...
        $tags   = apply_filters('gratora.social_meta', $this->tagsFor($campaign, $pageId), [
            'campaign' => $campaign,
            'page_id'  => $pageId,
        ]);
     ...
     * fundraiser/team/start layouts) carry _gratora_campaign_id too, so the page
     ...
     * gratora.social_meta filter then reshapes the tags for them.
     *
     * @since 1.0.0
     */
    private function currentCampaign(): ?Campaign
    {
        ...
        $campaignId = (int) get_post_meta($pageId, '_gratora_campaign_id', true);
        if ($campaignId <= 0) {
            return null;
        }
    ...
    private function enabled(): bool
    {
        ...
        return (bool) apply_filters('gratora.social_meta.enabled', ! $seoActive);
    }
