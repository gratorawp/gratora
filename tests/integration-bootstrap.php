 ...
 *
 * First-time setup:
 *   bin/install-wp-tests.sh gratora_test root '' localhost
 */
...
$tests_dir = getenv('WP_TESTS_DIR') ?: (static function (): string {
    ...
    $candidates = array_filter([
        $home !== '' ? $home . '/.gratora-wp-tests/wordpress-tests-lib' : null,
...
if (! file_exists($tests_dir . '/wp-tests-config.php')) {
    ...
    fwrite(STDERR, "Install it once, then every Gratora repo on this machine finds it:\n");
    fwrite(STDERR, "  composer test:setup <db-name> <db-user> <db-pass> [db-host] [wp-version]\n\n");
    fwrite(STDERR, "e.g.  composer test:setup gratora_tests root '' 127.0.0.1 latest\n");
...
// IdentityHasher that read wp_gratora_system_settings, so the schema must exist
// before boot - hence migrating here rather than on wp_loaded.
tests_add_filter('muplugins_loaded', static function (): void {
    require dirname(__DIR__) . '/gratora.php';
    \Gratora\Foundation\Plugin::migrateSchema();
});
