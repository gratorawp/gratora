...
final class AdminDonationActionsTest extends IntegrationTestCase
{
    public function test_show_returns_full_detail_with_donor_receipts_and_refunds(): void
    {
        ...
        $res = $this->get("/gratora/v1/admin/donations/{$reference}");
    ...
    {
        $res = $this->get('/gratora/v1/admin/donations/GRATORA-1999-99999');
    ...
    public function test_refund_full_amount_returns_refund_payload_and_flips_status(): void
    {
        ...
        $res = $this->post("/gratora/v1/admin/donations/{$reference}/refund", [
            'amount_cents' => 5000,
        ...
        $detail = $this->get("/gratora/v1/admin/donations/{$reference}")->get_data();
    ...
    public function test_refund_returns_422_when_donation_not_paid(): void
    {
        ...
        $res = $this->post("/gratora/v1/admin/donations/{$reference}/refund", ['amount_cents' => 1000]);

        $this->assertSame(422, $res->get_status());
        $this->assertSame('gratora_refund_failed', $res->get_data()['code']);
    }

    public function test_refund_404s_unknown_reference(): void
    {
        $res = $this->post('/gratora/v1/admin/donations/GRATORA-1999-99999/refund', ['amount_cents' => 1000]);
        $this->assertSame(404, $res->get_status());
    }
    ...
    public function test_resend_receipt_queues_async_job_and_resends_email(): void
    {
        ...
        $res = $this->post("/gratora/v1/admin/donations/{$reference}/resend-receipt", []);
        ...
        $receipts = self::$wpdb->get_results(
            "SELECT id FROM " . self::$prefix . "gratora_receipts"
        );
    ...
    public function test_resend_receipt_returns_422_for_unpaid_donation(): void
    {
        ...
        $res = $this->post("/gratora/v1/admin/donations/{$reference}/resend-receipt", []);
        $this->assertSame(422, $res->get_status());
        $this->assertSame('gratora_resend_unavailable', $res->get_data()['code']);
    }
    ...
    public function test_export_csv_includes_header_and_one_row_per_match(): void
    {
        ...
        $csv = $this->captureCsv('/gratora/v1/admin/donations/export.csv');
    ...
    public function test_export_csv_respects_search_filter(): void
    {
        ...
        $csv = $this->captureCsv('/gratora/v1/admin/donations/export.csv', ['search' => 'sarah@example.com']);
    ...
    public function test_export_csv_respects_gateway_and_test_filters(): void
    {
        ...
        $csv = $this->captureCsv('/gratora/v1/admin/donations/export.csv', ['gateway' => 'offline']);
        ...
        $csv = $this->captureCsv('/gratora/v1/admin/donations/export.csv', ['is_test' => 'false']);
        $this->assertStringContainsString('live@example.com', $csv);
    ...
    public function test_stats_excludes_test_money_from_raised_by_default(): void
    {
        ...
        $stats = $this->get('/gratora/v1/admin/donations/stats')->get_data();
        ...
        $stats = $this->get('/gratora/v1/admin/donations/stats', ['is_test' => 'true'])->get_data();
    ...
    private function seedPaidDonation(array $args = []): string
    {
        ...
        $d = Donation::make();
        $d->reference    = 'GRATORA-SEED-' . substr(md5($email . $amount), 0, 8);
        $d->donor_id     = $donor->id;
