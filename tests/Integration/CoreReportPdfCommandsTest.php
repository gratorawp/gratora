 ...
 * one-pager (aggregate only, gratora_view_reports) and a donor year-end tax
 * statement (PII, gratora_view_donors). The commands only mint a nonce-signed
...
final class CoreReportPdfCommandsTest extends IntegrationTestCase
{
    ...
    public function test_manifest_lists_report_document_commands_as_non_mutating(): void
    {
        ...
        $this->assertSame('gratora_view_reports', $byId['report.campaign_pdf']['capability']);
        $this->assertSame('gratora_view_donors', $byId['donor.tax_statement_pdf']['capability']);
    }
    ...
    public function test_campaign_pdf_route_streams_pdf_for_authed_admin(): void
    {
        ...
        $request  = new WP_REST_Request('GET', "/gratora/v1/reports/campaign/{$campaignId}/pdf");
    ...
    public function test_tax_statement_route_streams_pdf_for_authed_admin(): void
    {
        ...
        $year     = self::STATEMENT_YEAR;
        $request  = new WP_REST_Request('GET', "/gratora/v1/reports/donor/{$donorId}/tax-statement/{$year}");
        $response = rest_do_request($request);
        $this->assertSame(200, $response->get_status());
    ...
    public function test_campaign_pdf_route_denied_without_reports_cap(): void
    {
        ...
        $status = rest_do_request(new WP_REST_Request('GET', "/gratora/v1/reports/campaign/{$campaignId}/pdf"))->get_status();
        $this->assertContains($status, [401, 403]);
    }

    public function test_tax_statement_route_denied_without_donors_cap(): void
    {
        // A subscriber holds neither manage_options nor gratora_view_donors. Admins
        // hold gratora_view_donors implicitly (Capabilities::grantMetaCaps grants the
        ...
        $year   = self::STATEMENT_YEAR;
        $status = rest_do_request(new WP_REST_Request('GET', "/gratora/v1/reports/donor/{$donorId}/tax-statement/{$year}"))->get_status();
        $this->assertContains($status, [401, 403]);
    }
    ...
    private function seedCampaignWithDonation(): int
    {
        ...
        $this->seedPaidDonation((int) $campaign->id, 200_000, gmdate('Y-m-d H:i:s'), 'GRATORA-CMP-' . uniqid());
