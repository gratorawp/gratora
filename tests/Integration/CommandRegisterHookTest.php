...
/**
 * The giveflow.commands.register broadcast fires once, from Plugin::boot after all
...
final class CommandRegisterHookTest extends IntegrationTestCase
{
    ...
    {
        $this->assertGreaterThanOrEqual(1, did_action('giveflow.commands.register'));
    }
    ...
    public function test_handlers_receive_the_shared_populated_registry(): void
    {
        ...
        add_action('giveflow.commands.register', static function ($registry) use (&$seen): void {
            $seen = $registry;
            $registry->register(new \GiveFlow\Foundation\Commands\Command(
                'test.late_pack',
        ...
        do_action('giveflow.commands.register', $container->get(CommandRegistry::class), $container);
        ...
        remove_all_filters('giveflow.commands.register');
    }
}
