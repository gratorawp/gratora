...
final class PortalDonationPayloadTest extends IntegrationTestCase
{
    ...
    public function test_a_give_again_link_says_which_currency_its_amount_is_in(): void
    {
        ...
        $this->assertSame('500000', (string) ($query['gratora_amount'] ?? ''));
        $this->assertSame(
            'JPY',
            (string) ($query['gratora_currency'] ?? ''),
    ...
    private function refund(string $reference, int $amountCents): void
    {
        ...
        try {
            $req = new WP_REST_Request('POST', "/gratora/v1/admin/donations/{$reference}/refund");
            $req->set_header('content-type', 'application/json');
    ...
    {
        return (array) $this->asDonor($reference, "/gratora/v1/portal/donations/{$reference}");
    }

    /** @return array<string,mixed> */
    private function listDonations(string $reference): array
    {
        $rows = (array) $this->asDonor($reference, '/gratora/v1/portal/donations')['items'];
        foreach ($rows as $row) {
