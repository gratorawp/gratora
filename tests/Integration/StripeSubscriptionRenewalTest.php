...
final class StripeSubscriptionRenewalTest extends IntegrationTestCase
{
    ...
    public function test_invoice_payment_succeeded_creates_renewal_donation(): void
    {
        ...
        $renewedFired = false;
        add_action('gratora.recurring.renewed', function () use (&$renewedFired): void {
            $renewedFired = true;
        });
        ...
        $this->assertTrue($renewedFired, 'gratora.recurring.renewed action fired');
    ...
    public function test_invoice_payment_succeeded_is_idempotent(): void
    {
        ...
        $fresh = \Gratora\Recurring\RecurringPlan::query()->where('id', $plan->id)->get();
    ...
    public function test_a_redelivered_failure_counts_once_and_notifies_once(): void
    {
        ...
        $notified = [];
        add_action('gratora.recurring.renewal_failed', static function ($p, $ctx) use (&$notified): void {
    ...
    public function test_invoice_payment_failed_fires_the_renewal_failed_action(): void
    {
        ...
        $seen = [];
        add_action('gratora.recurring.renewal_failed', function ($p, $ctx) use (&$seen): void {
    ...
    public function test_a_retry_does_not_mail_the_donor_again(): void
    {
        ...
        $this->assertSame(
            2,
            (int) \Gratora\Recurring\RecurringPlan::query()->find('id', (int) $plan->id)->failed_renewals_count,
    ...
    public function test_a_failed_renewal_is_recorded_for_reporting(): void
    {
        ...
        $this->assertSame(
            1,
            (int) \Gratora\Analytics\Event::query()->where('type', 'recurring.failed')->count(),
    ...
    public function test_subscription_deleted_marks_plan_cancelled_and_fires_event(): void
    {
        ...
        $cancelFired = false;
        add_action('gratora.recurring.cancelled', function () use (&$cancelFired): void {
            $cancelFired = true;
        });
    ...
    private function seedPlan(): RecurringPlan
    {
        ...
        $donorService = \Gratora\Foundation\Plugin::instance()->container
