...
final class AdminFormsTest extends IntegrationTestCase
{
    ...
    public function test_index_returns_the_auto_created_default_form(): void
    {
        ...
        $res = $this->get('/gratora/v1/admin/forms');
    ...
    {
        $res = $this->post('/gratora/v1/admin/forms', [
            'title' => 'No campaign',
        ]);
    ...
    {
        $this->post('/gratora/v1/admin/forms', ['title' => 'My form', 'campaign_id' => $this->campaignId]);
        $second = $this->post('/gratora/v1/admin/forms', ['title' => 'My form', 'campaign_id' => $this->campaignId])->get_data();
    ...
    {
        $created = $this->post('/gratora/v1/admin/forms', [
            'title'       => 'With blocks',
            'campaign_id' => $this->campaignId,
            'blocks'      => '<!-- wp:paragraph --><p>Hi</p><!-- /wp:paragraph -->',
        ])->get_data();

        $res = $this->get("/gratora/v1/admin/forms/{$created['id']}");
        $this->assertSame(200, $res->get_status());
    ...
    {
        $res = $this->get('/gratora/v1/admin/forms/9999');
    ...
    public function test_update_form_partial_only_touches_supplied_fields(): void
    {
        ...
        $res = $this->put("/gratora/v1/admin/forms/{$created['id']}", [
            'title' => 'Renamed',
        ]);
        $this->assertSame(200, $res->get_status());

        $reloaded = $this->get("/gratora/v1/admin/forms/{$created['id']}")->get_data();
    ...
    public function test_update_form_publishes_and_stamps_published_at(): void
    {
        ...
        $res = $this->put("/gratora/v1/admin/forms/{$created['id']}", ['status' => 'published']);
        $this->assertSame(200, $res->get_status());
    ...
    {
        $a = $this->post('/gratora/v1/admin/forms', ['title' => 'Alpha form', 'campaign_id' => $this->campaignId])->get_data();
        $b = $this->post('/gratora/v1/admin/forms', ['title' => 'Beta form',  'campaign_id' => $this->campaignId])->get_data();

        $res = $this->put("/gratora/v1/admin/forms/{$b['id']}", ['slug' => $a['slug']]);
        $this->assertSame(422, $res->get_status());
        $this->assertSame('gratora_invalid_input', $res->get_data()['code']);
    }
    ...
    {
        $created = $this->post('/gratora/v1/admin/forms', ['title' => 'Disposable', 'campaign_id' => $this->campaignId])->get_data();

        $res = $this->deleteReq("/gratora/v1/admin/forms/{$created['id']}");
        $this->assertSame(200, $res->get_status());
        $this->assertTrue($res->get_data()['deleted']);

        $this->assertSame(404, $this->get("/gratora/v1/admin/forms/{$created['id']}")->get_status());
    }
    ...
    public function test_delete_blocks_the_campaign_default_form(): void
    {
        ...
        $campaign = \Gratora\Campaigns\Campaign::query()->find('id', $this->campaignId);
        ...
        $res = $this->deleteReq("/gratora/v1/admin/forms/{$defaultFormId}");
        $this->assertSame(422, $res->get_status());
        $this->assertSame('gratora_form_delete_blocked', $res->get_data()['code']);

        // Form still exists.
        $this->assertSame(200, $this->get("/gratora/v1/admin/forms/{$defaultFormId}")->get_status());
    }

    public function test_search_filters_by_title_substring(): void
    {
        $this->post('/gratora/v1/admin/forms', ['title' => 'Summer appeal',     'campaign_id' => $this->campaignId]);
        $this->post('/gratora/v1/admin/forms', ['title' => 'Winter appeal',     'campaign_id' => $this->campaignId]);
        $this->post('/gratora/v1/admin/forms', ['title' => 'Spring fundraiser', 'campaign_id' => $this->campaignId]);

        $res = $this->get('/gratora/v1/admin/forms', ['search' => 'appeal']);
    ...
    {
        $blocks = '<!-- wp:gratora/donation-amount {"presets":[500,1500],"currency":"USD"} /-->'
                . '<!-- wp:gratora/submit-button {"label":"Send"} /-->';

        $res = $this->post('/gratora/v1/admin/forms/preview', ['blocks' => $blocks]);
        $this->assertSame(200, $res->get_status());
    ...
    {
        $res = $this->post('/gratora/v1/admin/forms/preview', []);
        $this->assertSame(200, $res->get_status());
        $this->assertStringContainsString('gratora-donation-form--blocks', $res->get_data()['html']);
    }


    private function createCampaign(): int
    {
        $res = $this->post('/gratora/v1/admin/campaigns', ['title' => 'Campaign 1', 'status' => 'published']);
