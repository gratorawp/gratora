...
final class AdminCapabilityGateTest extends IntegrationTestCase
{
    ...
    public function test_view_donations_reaches_donations_not_donors(): void
    {
        ...
        $this->assertAllowed($this->status('GET', '/gratora/v1/admin/donations'), 'donations viewer sees donations');
        $this->assertForbidden($this->status('GET', '/gratora/v1/admin/donors'), 'donations viewer blocked from donors');
    }

    public function test_view_donors_reaches_donors_not_donations(): void
    {
        $this->actAs(['gratora_view_donors']);
        $this->assertAllowed($this->status('GET', '/gratora/v1/admin/donors'), 'donor viewer sees donors');
        $this->assertForbidden($this->status('GET', '/gratora/v1/admin/donations'), 'donor viewer blocked from donations');
    }
    ...
    public function test_manage_options_bypasses_all_gates(): void
    {
        ...
        $this->assertAllowed($this->status('GET', '/gratora/v1/admin/donors'), 'admin sees donors');
        $this->assertAllowed($this->status('GET', '/gratora/v1/admin/donations'), 'admin sees donations');
        $this->assertAllowed($this->status('GET', '/gratora/v1/admin/campaigns'), 'admin sees campaigns');
        $this->assertAllowed($this->status('GET', '/gratora/v1/admin/dashboard'), 'admin sees dashboard');
        $this->assertAllowed($this->status('GET', '/gratora/v1/admin/settings/general'), 'admin sees settings');
        ...
        $this->assertAllowed($this->status('GET', '/gratora/v1/admin/exports/donors.csv'), 'admin exports donors');
        $this->assertAllowed($this->status('GET', '/gratora/v1/admin/exports/revenue.csv'), 'admin exports revenue');
        $this->assertAllowed($this->status('GET', '/gratora/v1/admin/exports/options'), 'admin reads export options');
    }

    public function test_settings_requires_settings_cap(): void
    {
        $this->actAs(['gratora_view_donations']);
        $this->assertForbidden($this->status('GET', '/gratora/v1/admin/settings/general'), 'donations viewer blocked from settings');

        $this->actAs(['gratora_manage_settings']);
        $this->assertAllowed($this->status('GET', '/gratora/v1/admin/settings/general'), 'settings manager reaches settings');
    }

    public function test_redact_requires_redact_cap_not_just_view(): void
    {
        // view_donors alone cannot redact; the per-route gate needs gratora_redact_donors.
        $this->actAs(['gratora_view_donors']);
        $this->assertForbidden(
            $this->status('POST', '/gratora/v1/admin/donors/999999/redact', ['confirmation' => 'x']),
            'donor viewer cannot redact'
        );

        // With the cap the gate lets it through (404/422 for the missing donor, but not forbidden).
        $this->actAs(['gratora_view_donors', 'gratora_redact_donors']);
        $this->assertAllowed(
            $this->status('POST', '/gratora/v1/admin/donors/999999/redact', ['confirmation' => 'x']),
    ...
    public function test_managing_roles_requires_full_admin_not_just_settings_cap(): void
    {
        ...
        $this->assertAllowed(
            $this->status('PUT', '/gratora/v1/admin/settings/general', ['organization_name' => 'X']),
            'settings manager edits general settings'
        );
        // ...but must not rewrite the role->capability mapping (privilege escalation),
        $this->assertForbidden(
            $this->status('PUT', '/gratora/v1/admin/settings/roles', ['mapping' => ['subscriber' => ['gratora_refund_donations']]]),
        ...
        $this->assertForbidden(
            $this->status('POST', '/gratora/v1/admin/tools/import', ['settings' => []]),
            'settings manager cannot import a settings bundle'
        );
        $this->assertForbidden(
            $this->status('GET', '/gratora/v1/admin/tools/export'),
    ...
    public function test_bulk_exports_carry_the_capability_of_the_data_not_the_screen(): void
    {
        ...
        $this->actAs(['gratora_view_donors']);
        $this->assertForbidden(
            $this->status('GET', '/gratora/v1/admin/exports/donors.csv'),
            'viewing donors does not carry bulk export'
        );

        $this->actAs(['gratora_export_donors']);
        $this->assertAllowed(
            $this->status('GET', '/gratora/v1/admin/exports/donors.csv'),
        ...
        $this->assertForbidden(
            $this->status('GET', '/gratora/v1/admin/exports/revenue.csv'),
        ...
        // weaker cap made gratora_export_donors decorative.
        $this->actAs(['gratora_view_donations']);
        $csv = $this->body('GET', '/gratora/v1/admin/donations/export.csv');
        ...
        $this->actAs(['gratora_view_donations', 'gratora_export_donors']);
        $this->assertStringContainsString(
            'Donor email',
            $this->body('GET', '/gratora/v1/admin/donations/export.csv'),
            'holding the export capability restores the columns'
        );

        $this->actAs(['gratora_view_reports']);
        $this->assertAllowed(
            $this->status('GET', '/gratora/v1/admin/exports/revenue.csv'),
    ...
    public function test_full_admin_can_manage_roles(): void
    {
        ...
        $this->assertAllowed(
            $this->status('PUT', '/gratora/v1/admin/settings/roles', ['mapping' => []]),
