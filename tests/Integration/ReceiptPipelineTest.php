...
final class ReceiptPipelineTest extends IntegrationTestCase
{
    public function test_paid_donation_schedules_receipt_job_and_running_it_produces_receipt_row(): void
    {
        ...
        $pending = self::$wpdb->get_results(
            "SELECT hook FROM " . self::$prefix . "actionscheduler_actions WHERE hook = 'giveflow.async.issue_receipt' AND status = 'pending'"
        );
        $this->assertCount(1, $pending);

        $this->runPendingAsyncJobs();

        $receipt = self::$wpdb->get_row("SELECT * FROM " . self::$prefix . "giveflow_receipts");
    ...
    public function test_running_pipeline_calls_wp_mail_with_pdf_attachment(): void
    {
        ...
        $this->assertMatchesRegularExpression('#/giveflow/v1/receipts/\d+/download\?token=[a-f0-9]+#', $mail['message']);
    }
    ...
    public function test_receipt_issued_event_fires_with_donor_and_receipt_link(): void
    {
        ...
        $event = self::$wpdb->get_row("SELECT * FROM " . self::$prefix . "giveflow_events WHERE type = 'receipt.issued'");
    ...
    public function test_pipeline_is_idempotent_on_repeat_invocation(): void
    {
        $this->driveDonationToPaid();
        $this->runPendingAsyncJobs();
        $firstCount = (int) self::$wpdb->get_var("SELECT COUNT(*) FROM " . self::$prefix . "giveflow_receipts");
        ...
        do_action('giveflow.async.issue_receipt', ['donation_id' => 1]);

        $secondCount = (int) self::$wpdb->get_var("SELECT COUNT(*) FROM " . self::$prefix . "giveflow_receipts");
