...
final class AdminFundsTest extends IntegrationTestCase
{
    ...
    {
        $res = $this->get('/giveflow/v1/admin/funds');
    ...
    {
        $res = $this->post('/giveflow/v1/admin/funds', ['code' => 'building']);
    ...
    {
        $res = $this->post('/giveflow/v1/admin/funds', [
            'code'          => 'Building',
            'name'          => 'Building Fund',
    ...
    {
        $this->post('/giveflow/v1/admin/funds', ['code' => 'general', 'name' => 'General']);
        $res = $this->post('/giveflow/v1/admin/funds', ['code' => 'general', 'name' => 'Other']);
        $this->assertSame(422, $res->get_status());
        $this->assertSame('giveflow_invalid_input', $res->get_data()['code']);
    }

    public function test_show_404s_unknown_fund(): void
    {
        $res = $this->get('/giveflow/v1/admin/funds/99999');
    ...
    {
        $created = $this->post('/giveflow/v1/admin/funds', [
            'code' => 'scholar', 'name' => 'Scholarship',
        ])->get_data();

        $res = $this->put("/giveflow/v1/admin/funds/{$created['id']}", ['name' => 'Scholarship Fund']);
        $this->assertSame(200, $res->get_status());
    ...
    {
        $fund = $this->post('/giveflow/v1/admin/funds', [
            'code' => 'general', 'name' => 'General', 'is_default' => true,
        ])->get_data();

        $res = $this->deleteReq("/giveflow/v1/admin/funds/{$fund['id']}");
        $this->assertSame(422, $res->get_status());
        $this->assertSame('giveflow_fund_delete_blocked', $res->get_data()['code']);
    }
    ...
    public function test_unused_fund_is_hard_deleted(): void
    {
        ...
        $this->assertSame(404, $this->get("/giveflow/v1/admin/funds/{$fund['id']}")->get_status());
    }

    public function test_referenced_fund_is_deactivated_not_deleted(): void
    {
        $fund     = $this->post('/giveflow/v1/admin/funds', ['code' => 'restricted', 'name' => 'Restricted'])->get_data();
        $campaign = $this->post('/giveflow/v1/admin/campaigns', ['title' => 'Appeal'])->get_data();
        $this->put("/giveflow/v1/admin/campaigns/{$campaign['id']}", ['default_fund_id' => $fund['id']]);
        ...
        $reloaded = $this->get("/giveflow/v1/admin/funds/{$fund['id']}");
    ...
    {
        $fund     = $this->post('/giveflow/v1/admin/funds', ['code' => 'formfund', 'name' => 'Form Fund'])->get_data();
        $campaign = $this->post('/giveflow/v1/admin/campaigns', ['title' => 'Has Form'])->get_data();
    ...
    {
        $fund     = $this->post('/giveflow/v1/admin/funds', ['code' => 'old', 'name' => 'Old'])->get_data();
        $target   = $this->post('/giveflow/v1/admin/funds', ['code' => 'new', 'name' => 'New'])->get_data();
        $campaign = $this->post('/giveflow/v1/admin/campaigns', ['title' => 'Appeal'])->get_data();
        $this->put("/giveflow/v1/admin/campaigns/{$campaign['id']}", ['default_fund_id' => $fund['id']]);
        ...
        $donation->reference = 'GIVEFLOW-FUND-MOVE'; $donation->donor_id = 1;
        ...
        $res = $this->deleteReq("/giveflow/v1/admin/funds/{$fund['id']}", ['reassign_to' => $target['id']]);
        $this->assertSame(202, $res->get_status());
        $this->assertSame('reassign_queued', $res->get_data()['action']);

        $pending = $this->get("/giveflow/v1/admin/funds/{$fund['id']}");
        ...
        $reloadedCampaign = $this->get("/giveflow/v1/admin/campaigns/{$campaign['id']}")->get_data();
        $this->assertSame($target['id'], (int) $reloadedCampaign['default_fund_id'],
            'Campaign default fund repointed to the target');

        $this->assertSame((int) $target['id'], (int) \GiveFlow\Forms\Form::query()->where('id', $form->id)->get()->default_fund_id,
            'Form default fund repointed to the target');
        $this->assertSame((int) $target['id'], (int) \GiveFlow\Recurring\RecurringPlan::query()->where('id', $plan->id)->get()->fund_id,
            'Recurring plan fund repointed to the target');

        $targetFund = \GiveFlow\Funds\Fund::query()->where('id', (int) $target['id'])->get();
    ...
    {
        $fund     = $this->post('/giveflow/v1/admin/funds', ['code' => 'src', 'name' => 'Src'])->get_data();
        $campaign = $this->post('/giveflow/v1/admin/campaigns', ['title' => 'Appeal'])->get_data();
        $this->put("/giveflow/v1/admin/campaigns/{$campaign['id']}", ['default_fund_id' => $fund['id']]);

        $missing = $this->deleteReq("/giveflow/v1/admin/funds/{$fund['id']}", ['reassign_to' => 999999]);
        $this->assertSame(422, $missing->get_status());
        $this->assertSame('giveflow_invalid_input', $missing->get_data()['code']);

        $self = $this->deleteReq("/giveflow/v1/admin/funds/{$fund['id']}", ['reassign_to' => $fund['id']]);
        $this->assertSame(422, $self->get_status());
        $this->assertSame('giveflow_invalid_input', $self->get_data()['code']);
    }

    public function test_orphaned_reassignment_is_self_healed_on_list_load(): void
    {
        $fund     = $this->post('/giveflow/v1/admin/funds', ['code' => 'orphan', 'name' => 'Orphan'])->get_data();
        $target   = $this->post('/giveflow/v1/admin/funds', ['code' => 'keep', 'name' => 'Keep'])->get_data();
        $campaign = $this->post('/giveflow/v1/admin/campaigns', ['title' => 'Appeal'])->get_data();
        $this->put("/giveflow/v1/admin/campaigns/{$campaign['id']}", ['default_fund_id' => $fund['id']]);
        ...
        // re-queued it).
        \GiveFlow\Funds\FundReassignmentJob::markPending((int) $fund['id'], (int) $target['id']);

        $pendingActions = static fn (): int => (int) self::$wpdb->get_var(
            "SELECT COUNT(*) FROM " . self::$prefix . "actionscheduler_actions"
            . " WHERE hook = '" . \GiveFlow\Funds\FundReassignmentJob::HOOK . "' AND status = 'pending'"
        );
        ...
        $this->get('/giveflow/v1/admin/funds');
        ...
        $this->assertArrayNotHasKey((int) $fund['id'], \GiveFlow\Funds\FundReassignmentJob::pending(),
            'Pending marker cleared, so the badge resolves');
        $reCampaign = $this->get("/giveflow/v1/admin/campaigns/{$campaign['id']}")->get_data();
    ...
    {
        for ($i = 1; $i <= 6; $i++) {
            $this->post('/giveflow/v1/admin/funds', ['code' => "fund-$i", 'name' => "Fund $i"]);
        }

        $res = $this->get('/giveflow/v1/admin/funds', ['per_page' => 5, 'page' => 1]);
        $this->assertSame(200, $res->get_status());
    ...
    {
        $this->post('/giveflow/v1/admin/funds', ['code' => 'roof', 'name' => 'Roof Repair']);
        $this->post('/giveflow/v1/admin/funds', ['code' => 'general', 'name' => 'General']);

        $res = $this->get('/giveflow/v1/admin/funds', ['search' => 'roof']);
    ...
    {
        $a = $this->post('/giveflow/v1/admin/funds', ['code' => 'a', 'name' => 'A'])->get_data();

        $res = $this->put("/giveflow/v1/admin/funds/{$a['id']}", ['parent_fund_id' => $a['id']]);
        $this->assertSame(422, $res->get_status());
        $this->assertSame('giveflow_invalid_input', $res->get_data()['code']);
    }
    ...
    {
        $parent = $this->post('/giveflow/v1/admin/funds', ['code' => 'parent', 'name' => 'Parent'])->get_data();
        $child  = $this->post('/giveflow/v1/admin/funds', [
    ...
    {
        $parent = $this->post('/giveflow/v1/admin/funds', ['code' => 'parent', 'name' => 'Parent'])->get_data();
        $this->post('/giveflow/v1/admin/funds', [
            'code' => 'child', 'name' => 'Child', 'parent_fund_id' => $parent['id'],
        ]);
        $other = $this->post('/giveflow/v1/admin/funds', ['code' => 'other', 'name' => 'Other'])->get_data();

        // Parent has a sub-fund, so it cannot be nested under Other (and this
        // is exactly the A->B, B->A cycle case).
        $res = $this->put("/giveflow/v1/admin/funds/{$parent['id']}", ['parent_fund_id' => $other['id']]);
        $this->assertSame(422, $res->get_status());
        $this->assertSame('giveflow_invalid_input', $res->get_data()['code']);
    }
    ...
    public function test_stats_returns_org_wide_aggregates(): void
    {
        ...
        $inactive = $this->post('/giveflow/v1/admin/funds', [
            'code' => 'old', 'name' => 'Old',
        ])->get_data();
        $this->put("/giveflow/v1/admin/funds/{$inactive['id']}", ['is_active' => false]);

        $stats = $this->get('/giveflow/v1/admin/funds/stats')->get_data();
    ...
    {
        $parent = $this->post('/giveflow/v1/admin/funds', [
        ...
        \GiveFlow\Funds\Fund::query()->where('id', (int) $child['id'])
            ->update(['raised_cents' => 7000, 'donations_count' => 2]);

        $byId = [];
        foreach ($this->get('/giveflow/v1/admin/funds')->get_data() as $row) {
            $byId[(int) $row['id']] = $row;
        }
        ...
        $this->assertSame(7000, $this->get('/giveflow/v1/admin/funds/stats')->get_data()['raised_cents'],
    ...
    public function test_deletable_flag_mirrors_the_delete_guard(): void
    {
        ...
        $free = $this->post('/giveflow/v1/admin/funds', ['code' => 'free', 'name' => 'Free'])->get_data();
        ...
        $used     = $this->post('/giveflow/v1/admin/funds', ['code' => 'used', 'name' => 'Used'])->get_data();
        $campaign = $this->post('/giveflow/v1/admin/campaigns', ['title' => 'Appeal'])->get_data();
        $this->put("/giveflow/v1/admin/campaigns/{$campaign['id']}", ['default_fund_id' => $used['id']]);

        // A fund reached only by a test donation is likewise blocked.
        $tested = $this->post('/giveflow/v1/admin/funds', ['code' => 'tested', 'name' => 'Tested'])->get_data();
        $now = gmdate('Y-m-d H:i:s');
        $d = \GiveFlow\Donations\Donation::make();
        $d->reference = 'GIVEFLOW-DEL-TEST'; $d->donor_id = 1;
        ...
        $default = $this->post('/giveflow/v1/admin/funds', [
            'code' => 'main', 'name' => 'Main', 'is_default' => true,
        ])->get_data();
        $this->assertFalse($this->get("/giveflow/v1/admin/funds/{$default['id']}")->get_data()['deletable']);
    }
