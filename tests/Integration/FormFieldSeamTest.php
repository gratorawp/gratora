...
use GiveFlow\Forms\Shortcode\FormFieldAssets;
use WP_REST_Request;
...
final class FormFieldSeamTest extends IntegrationTestCase
{
    ...
    {
        add_filter('giveflow.form.block_field', static function ($item, string $name, array $attrs) {
    ...
    {
        add_filter('giveflow.form.block_field', static function ($item, string $name) {
    ...
    public function test_the_donation_form_runtime_depends_on_the_field_registry(): void
    {
        ...
        $slug = $this->publishedForm('<!-- wp:giveflow/submit-button /-->');
        do_shortcode('[giveflow_donation_form slug="' . $slug . '"]');

        $runtime = $wp_scripts->registered['giveflow-donation-form-runtime'] ?? null;
    ...
    {
        $html = do_shortcode('[giveflow_donation_form slug="' . $this->publishedForm($blocks) . '"]');
    ...
    {
        $config = $this->configFor(
            '<!-- wp:giveflow/donation-amount /--><!-- wp:giveflow/submit-button /-->'
        );
