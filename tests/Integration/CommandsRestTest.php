 ...
 * gratora_refund_donations is still denied donation.refund.
 */
final class CommandsRestTest extends IntegrationTestCase
{
    ...
    public function test_post_read_command_as_admin_returns_200(): void
    {
        ...
        // administrator carries manage_gratora only until the roles UI grants it.
        $reference = $this->driveDonationToPaid();
        $this->actAsAdminWithCap('gratora_view_donations');

        $res = $this->post('/gratora/v1/admin/commands/donation.get', [
    ...
    public function test_refund_denied_for_manage_options_user_without_fine_grained_cap(): void
    {
        ...
        // gratora_refund_donations cap (granted only via the roles mapping UI).
        $reference = $this->driveDonationToPaid();
        $this->actAsAdminWithoutCap('gratora_refund_donations');

        $res = $this->post('/gratora/v1/admin/commands/donation.refund', [
        ...
        $denied = self::$wpdb->get_results(
            "SELECT type FROM " . self::$prefix . "gratora_events WHERE type = 'command.denied'"
        );
    ...
    {
        $reference = $this->driveDonationToPaid();
        $this->actAsAdminWithCap('gratora_refund_donations');
        ...
        $refunds = self::$wpdb->get_var("SELECT COUNT(*) FROM " . self::$prefix . "gratora_refunds");
        $this->assertSame('0', (string) $refunds);
    }

    public function test_unknown_command_returns_404(): void
    {
        $res = $this->post('/gratora/v1/admin/commands/nope.missing', ['input' => []]);
