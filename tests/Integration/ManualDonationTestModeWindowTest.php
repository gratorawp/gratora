 ...
 * fires giveflow.donation.creating inside its own transaction, so every listener on
...
final class ManualDonationTestModeWindowTest extends IntegrationTestCase
{
    public function test_no_listener_ever_sees_a_recorded_cheque_as_a_test_donation(): void
    {
        ...
        $seen = [];
        add_action('giveflow.donation.creating', static function ($donation) use (&$seen): void {
            $seen[] = (bool) $donation->is_test;
        }, 1);

        $request = new WP_REST_Request('POST', '/giveflow/v1/admin/donations');
