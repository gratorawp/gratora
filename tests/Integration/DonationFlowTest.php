...
final class DonationFlowTest extends IntegrationTestCase
{
    public function test_post_donations_creates_donor_and_pending_donation(): void
    {
        ...
        $data = $res->get_data();
        $this->assertMatchesRegularExpression('/^DON-\d{4}-\d{5}$/', $data['reference']);
        ...
        $donor = self::$wpdb->get_row(
            "SELECT email_hash, first_name, last_name, country FROM " . self::$prefix . "giveflow_donors LIMIT 1"
        );
        ...
        $donation = self::$wpdb->get_row(self::$wpdb->prepare(
            "SELECT status, amount_cents, currency, gateway_intent_id, country FROM " . self::$prefix . "giveflow_donations WHERE reference = %s",
            $data['reference']
        ));
        ...
        $events = self::$wpdb->get_results(
            "SELECT type FROM " . self::$prefix . "giveflow_events ORDER BY id"
        );
    ...
    public function test_repeat_donor_does_not_create_duplicate_donor_row(): void
    {
        ...
        $donorCount = (int) self::$wpdb->get_var("SELECT COUNT(*) FROM " . self::$prefix . "giveflow_donors");
        $donationCount = (int) self::$wpdb->get_var("SELECT COUNT(*) FROM " . self::$prefix . "giveflow_donations");
    ...
    public function test_post_donations_rejects_bad_payloads(): void
    {
        $bad = [
            ...
            ['payload' => ['email' => 'x@y.com', 'amount_cents' => 5000, 'currency' => 'EUR', 'gateway' => 'nope'],   'code' => 'giveflow_invalid_gateway'],
        ];
     ...
     * rejected the request in core, before GiveFlow could log or recover from it,
    ...
    public function test_an_enormous_landing_url_costs_the_attribution_not_the_donation(): void
    {
        ...
        $blob = json_decode((string) self::$wpdb->get_var(self::$wpdb->prepare(
            'SELECT source_attribution FROM ' . self::$prefix . 'giveflow_donations WHERE reference = %s',
            $ref
        )), true);
    ...
    public function test_note_to_org_is_private_unless_the_donor_opts_in(): void
    {
        ...
        $form = $this->publishedFormOffering('<!-- wp:giveflow/comment /-->');
    ...
    public function test_zero_decimal_currency_rejects_fractional_amounts(): void
    {
        ...
        $this->assertSame('giveflow_invalid_amount', $bad->get_data()['code'] ?? null);
    ...
    public function test_explicit_active_fund_is_attributed(): void
    {
        ...
        $form = $this->publishedFormOffering('<!-- wp:giveflow/fund-picker /-->');
    ...
    private function publishedFormOffering(string $block): Form
    {
        ...
        $f->slug       = 'flow-' . uniqid();
        $f->status     = 'published';
        $f->blocks     = '<!-- wp:giveflow/donation-amount /--><!-- wp:giveflow/email /-->'
            . $block
    ...
    private function columnOf(string $reference, string $column): int
    {
        ...
        return (int) self::$wpdb->get_var(self::$wpdb->prepare(
            "SELECT {$column} FROM " . self::$prefix . "giveflow_donations WHERE reference = %s",
            $reference
        ));
    }
    ...
    public function test_post_donation_rejects_a_gateway_the_form_does_not_offer(): void
    {
        ...
        update_option('giveflow_gateway_config', ['offline' => ['enabled' => false]]);
        ...
        $this->assertSame(400, $res->get_status());
        $this->assertSame('giveflow_gateway_not_allowed', $res->get_data()['code'] ?? null);
