...
final class CurrencyFxTest extends IntegrationTestCase
{
    ...
    private function makeDefaultFund(): \Gratora\Funds\Fund
    {
        $f = \Gratora\Funds\Fund::make();
        $f->code       = 'genfx';
        $f->name       = 'General FX';
        $f->is_active  = true;
        $f->is_default = true;
    ...
    public function test_donation_snapshots_base_amount(): void
    {
        ...
        $row = self::$wpdb->get_row(self::$wpdb->prepare(
            "SELECT base_amount_cents, base_currency, fx_rate FROM " . self::$prefix . "gratora_donations WHERE reference = %s",
            $ref
        ));
    ...
    public function test_same_currency_is_one_to_one(): void
    {
        ...
        $row = self::$wpdb->get_row(self::$wpdb->prepare(
            "SELECT base_amount_cents, base_currency FROM " . self::$prefix . "gratora_donations WHERE reference = %s",
            $ref
        ));
    ...
    {
        return rest_do_request(new WP_REST_Request('GET', '/gratora/v1/admin/currency/fx'))->get_data();
    }

    private function fxPut(array $body): array
    {
        $r = new WP_REST_Request('PUT', '/gratora/v1/admin/currency/fx');
    ...
    {
        return rest_do_request(new WP_REST_Request('POST', '/gratora/v1/admin/currency/fx/fetch'))->get_data();
    }
}
