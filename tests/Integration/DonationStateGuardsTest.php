...
final class DonationStateGuardsTest extends IntegrationTestCase
{
    ...
    public function test_public_confirm_route_rejects_a_refunded_donation_with_422(): void
    {
        ...
        $req = new WP_REST_Request('POST', "/gratora/v1/donations/{$donation->reference}/confirm");
    ...
    public function test_admin_mark_failed_rejects_a_refunded_donation_with_422(): void
    {
        ...
        $req = new WP_REST_Request('POST', "/gratora/v1/admin/donations/{$donation->reference}/mark-failed");
        $req->set_header('content-type', 'application/json');
        $req->set_body('{}');
        $res = rest_do_request($req);
    ...
    {
        $first  = rest_do_request(new WP_REST_Request('POST', '/gratora/v1/webhooks/offline'));
        $second = rest_do_request(new WP_REST_Request('POST', '/gratora/v1/webhooks/offline'));
    ...
    {
        rest_do_request(new WP_REST_Request('POST', '/gratora/v1/webhooks/offline'));
        rest_do_request(new WP_REST_Request('POST', '/gratora/v1/webhooks/offline'));
    ...
    {
        return \Gratora\Foundation\Plugin::instance()->container->get(DonationRepository::class);
    }
}
