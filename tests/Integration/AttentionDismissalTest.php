...
final class AttentionDismissalTest extends IntegrationTestCase
{
    ...
    private function failDonations(int $howMany): void
    {
        ...
        for ($i = 0; $i < $howMany; $i++) {
            $d = Donation::make();
            $d->reference         = 'GIVEFLOW-F-' . bin2hex(random_bytes(4));
            $d->donor_id          = 1;
            $d->amount_cents      = 2500;
            $d->currency          = 'USD';
    ...
    public function test_the_routes_dismiss_and_restore_for_the_current_user(): void
    {
        ...
        $req = new WP_REST_Request('POST', '/giveflow/v1/admin/me/attention/dismiss');
        ...
        $req = new WP_REST_Request('POST', '/giveflow/v1/admin/me/attention/restore');
        $req->set_header('content-type', 'application/json');
