...
final class CampaignFundTransactionRollbackTest extends IntegrationTestCase
{
    ...
    private function pagesLinkedToCampaigns(): int
    {
        return (int) self::$wpdb->get_var(
            "SELECT COUNT(*) FROM " . self::$prefix . "posts p
             ...
             WHERE p.post_type = 'page' AND m.meta_key = '_fundkit_campaign_id'"
        );
    }
    ...
    private function defaultFundIds(): array
    {
        $ids = self::$wpdb->get_col(
            'SELECT id FROM ' . self::$prefix . 'fundkit_funds WHERE is_default = 1 ORDER BY id'
        );
    ...
    {
        $before = [
            'campaigns' => $this->countRows('fundkit_campaigns', '1=1'),
            'forms'     => $this->countRows('fundkit_forms', '1=1'),
        ...
        $this->whileQueryThrows(
            static fn (string $sql): bool => stripos($sql, 'UPDATE') === 0
                && stripos($sql, 'fundkit_campaigns') !== false,
            function (): void {
                try {
     ...
     * fundkit.form.created fires inside the campaign's transaction: an add-on
    ...
    public function test_an_add_on_refusing_the_form_hook_undoes_the_whole_campaign_create(): void
    {
        ...
        add_action('fundkit.form.created', $thrower);
        try {
            $this->campaigns()->create(['title' => 'Rollback Reef']);
            ...
            remove_action('fundkit.form.created', $thrower);
        }
    ...
    public function test_a_campaign_duplicate_that_does_not_throw_lands_copy_form_and_page(): void
    {
        ...
        $this->assertSame(
            ...
            [
                'campaign' => $this->countRows('fundkit_campaigns', 'id = ' . (int) $copy->id),
                'form'     => $this->countRows('fundkit_forms', 'id = ' . (int) $copy->default_form_id),
     ...
     * fundkit.form.deleted hook, which fires before either statement runs and so
    ...
    public function test_a_refused_campaign_delete_puts_its_forms_back(): void
    {
        ...
        $this->assertSame(1, $this->countRows('fundkit_forms', 'campaign_id = ' . (int) $campaign->id));
        ...
        $this->assertSame(
            ...
            [
                'campaign' => $this->countRows('fundkit_campaigns', 'id = ' . (int) $campaign->id),
                'forms'    => $this->countRows('fundkit_forms', 'campaign_id = ' . (int) $campaign->id),
    ...
    public function test_a_campaign_delete_that_does_not_throw_removes_campaign_form_and_page(): void
    {
        ...
        $this->assertSame(
            ...
            [
                'campaign' => $this->countRows('fundkit_campaigns', 'id = ' . (int) $campaign->id),
                'forms'    => $this->countRows('fundkit_forms', 'id = ' . $formId),
                'page'     => $this->countRows('posts', 'ID = ' . $pageId),
            ],
    ...
    public function test_a_failed_promotion_leaves_exactly_one_default_fund(): void
    {
        ...
        // Second UPDATE on fundkit_funds in the block: the first raised the new
        // default, so the old one is still up when this one is refused.
        $this->whileNthQueryThrows(
            static fn (string $sql): bool => stripos($sql, 'UPDATE') === 0
                && stripos($sql, 'fundkit_funds') !== false,
            2,
    ...
    public function test_a_failed_default_fund_create_leaves_no_row_and_the_old_default_standing(): void
    {
        ...
        $this->assertSame(
            0,
            $this->countRows('fundkit_funds', "code = 'building'"),
    ...
    public function test_a_campaign_create_that_does_not_throw_lands_campaign_form_and_page(): void
    {
        ...
        $this->assertSame(
            1,
            $this->countRows('fundkit_campaigns', 'id = ' . (int) $campaign->id),
            'the campaign row was not written'
        );
        $this->assertGreaterThan(0, (int) $campaign->default_form_id);
        $this->assertSame(
            1,
            $this->countRows('fundkit_forms', 'id = ' . (int) $campaign->default_form_id),
