 ...
 * whitelist, and the giveflow.settings.updated broadcast the FX snapshot and the
...
final class ToolsImportSettingsWriterTest extends IntegrationTestCase
{
    ...
    public function test_an_imported_file_cannot_change_the_base_currency_after_live_money(): void
    {
        ...
        $res = $this->post(['settings' => ['giveflow_currency_locale' => [
            'default_currency'     => 'EUR',
            'supported_currencies' => ['EUR'],
        ]]]);

        $this->assertSame(409, $res->get_status(), 'the import must refuse the write the settings screen refuses');
        $this->assertSame('giveflow_base_currency_locked', $res->as_error()->get_error_code());
    ...
    public function test_a_refused_group_does_not_stop_the_rest_of_the_file(): void
    {
        ...
        $res = $this->post(['settings' => [
            'giveflow_currency_locale' => ['default_currency' => 'EUR'],
            'giveflow_receipt_settings' => ['header_title' => 'Imported receipt'],
        ]]);
        ...
        $this->assertSame(
            'Imported receipt',
            (string) (get_option('giveflow_receipt_settings')['header_title'] ?? ''),
            'the groups that were allowed still landed'
        );

        $data = $res->as_error()->get_error_data();
        $this->assertSame(['giveflow_currency_locale'], array_keys((array) ($data['refused'] ?? [])));
    ...
    public function test_a_scalar_settings_payload_cannot_blank_the_base_currency(): void
    {
        ...
        // route.
        $res = $this->post(['settings' => ['giveflow_currency_locale' => 'EUR']]);
    ...
    public function test_an_allowed_base_currency_change_still_broadcasts(): void
    {
        ...
        $fx = get_option('giveflow_fx_rates');
    ...
    {
        $res = $this->post(['settings' => ['giveflow_privacy' => [
            'anonymize_ips' => false,
            'evil_key'      => 'planted',
        ]]]);
    ...
    {
        Plugin::instance()->container->get(SettingsService::class)
            ->update('roles', ['mapping' => ['author' => ['giveflow_view_donations']]]);
        $this->assertTrue(get_role('author')->has_cap('giveflow_view_donations'));

        $res = $this->post(['settings' => ['giveflow_roles' => [
            'mapping' => ['editor' => ['giveflow_view_donations']],
        ]]]);

        $this->assertSame(200, $res->get_status());
        $this->assertSame(
            ['editor' => ['giveflow_view_donations']],
            (array) (get_option('giveflow_roles')['mapping'] ?? []),
            'the mapping is replaced, not merged, or a role can never be taken off it'
        );
        $this->assertTrue(get_role('editor')->has_cap('giveflow_view_donations'));
        $this->assertFalse(get_role('author')->has_cap('giveflow_view_donations'));
    }

    public function test_an_option_no_group_declares_is_refused_rather_than_written(): void
    {
        add_filter('giveflow.settings.groups', static function (array $groups): array {
            unset($groups['consents']);

            return $groups;
        });

        $res = $this->post(['settings' => ['giveflow_consents' => ['purposes' => [['key' => 'planted']]]]]);
        ...
        $stored = get_option('giveflow_consents');
    ...
    public function test_a_restore_keeps_the_settings_of_a_gateway_this_site_has_not_registered(): void
    {
        ...
        $res = $this->post(['settings' => ['giveflow_gateway_config' => [
            'test_mode' => false,
