 ...
 * everywhere, so GIVEFLOW-2026-00007 exists on most of them and belongs to a
 ...
 * that had taken donations of its own, an org that set GiveFlow up on a new host
...
final class ImportReferenceCollisionTest extends IntegrationTestCase
{
    ...
    {
        $reference = 'GIVEFLOW-2026-09' . random_int(100, 999);
        ...
        $this->assertSame(
            0,
            (int) ($result['existing']['giveflow_donations'] ?? 0),
            'a different donation with the same number is not "already there"'
        );
        $this->assertSame(
            1,
            (int) ($result['dropped']['giveflow_donations']['reference_collision'] ?? 0),
    ...
    {
        $this->seedDonation('rerun-' . uniqid() . '@example.test', 'GIVEFLOW-2026-08' . random_int(100, 999), 1500);
