...
/**
 * @covers \Gratora\Foundation\License\LicenseRefusals
 */
final class LicenseRefusalsTest extends TestCase
{
    ...
    {
        $groups = LicenseRefusals::group($this->refused(['Gratora Events', 'invalid']));
    ...
    {
        $groups = LicenseRefusals::group($this->refused(['Gratora Events', 'over_limit']));
    ...
    {
        $groups = LicenseRefusals::group($this->refused(['Gratora Events', 'not_entitled']));
    ...
    {
        $groups = LicenseRefusals::group($this->refused(['Gratora Events', 'revoked']));
    ...
    public function test_each_refusal_reason_gets_its_own_message(): void
    {
        $groups = LicenseRefusals::group($this->refused(
            ['Gratora Events', 'not_entitled'],
            ['Gratora Tributes', 'over_limit'],
            ['Gratora Gift Aid', 'not_entitled'],
        ));

        $this->assertCount(2, $groups, 'two distinct reasons, two messages');

        $byStatus = array_column($groups, 'names', 'status');
        $this->assertSame('Gratora Events, Gratora Gift Aid', $byStatus['not_entitled']);
        $this->assertSame('Gratora Tributes', $byStatus['over_limit']);
    }

    public function test_the_reason_the_admin_can_act_on_is_listed_first(): void
    {
        $groups = LicenseRefusals::group($this->refused(
            ['Gratora Events', 'revoked'],
            ['Gratora Tributes', 'invalid'],
        ));
    ...
    {
        $groups = LicenseRefusals::group($this->refused(['Gratora Events', 'something_new']));

        $this->assertCount(1, $groups);
        $this->assertNotSame('', $groups[0]['headline']);
        $this->assertStringContainsString('Gratora Events', $groups[0]['headline']);
    }
}
