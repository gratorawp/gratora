...
final class UninstallEntryPointTest extends TestCase
{
    ...
    public function test_including_uninstall_leaves_the_eraser_able_to_run(): void
    {
        ...
        $eraser = new Gratora\Foundation\Uninstall\DataEraser();
    ...
    {
        $result = $this->runProbe(<<<'PHP'
        echo class_exists('Gratora\Campaigns\Campaign') ? 'CAMPAIGN=yes' : 'CAMPAIGN=no';
        PHP);
    ...
    /**
     * A site the erase throws on: an add-on listening on gratora.uninstall, or
    ...
    private function runProbe(string $body, array $opts = []): string
    {
        ...
        $script = "<?php\n"
            ...
            . "define('WP_UNINSTALL_PLUGIN', 'gratora/gratora.php');\n"
            . "\$GLOBALS['erased'] = false;\n"
            . "\$GLOBALS['switched'] = [];\n"
            . "\$GLOBALS['restored'] = 0;\n"
            . "function get_option(\$name, \$default = false) {\n"
            . "    return \$name === 'gratora_delete_data' ? {$optIn} : \$default;\n"
            . "}\n"
        ...
        $this->probe = sys_get_temp_dir() . '/gratora-uninstall-probe-' . bin2hex(random_bytes(6)) . '.php';
