...
namespace GiveFlow\Tests\Unit\Async;
...
final class AsyncDispatcherTest extends TestCase
{
    ...
    {
        $all = $GLOBALS['_giveflow_as_calls'] ?? [];
    ...
    public function test_enqueue_delegates_to_action_scheduler(): void
    {
        ...
        $d->enqueue('giveflow.test.hook', ['key' => 'value']);

        $calls = $this->calls('as_enqueue_async_action');
        $this->assertCount(1, $calls);
        $this->assertSame('giveflow.test.hook', $calls[0]['args'][0]);
        $this->assertSame(['key' => 'value'], $calls[0]['args'][1]);
        $this->assertSame('giveflow', $calls[0]['args'][2]);
    }

    public function test_schedule_delegates_with_timestamp(): void
    {
        $d = new AsyncDispatcher();
        $d->schedule('giveflow.test.hook', 1700000000, ['id' => 5]);

        $calls = $this->calls('as_schedule_single_action');
        $this->assertCount(1, $calls);
        $this->assertSame(1700000000, $calls[0]['args'][0]);
        $this->assertSame('giveflow.test.hook', $calls[0]['args'][1]);
        $this->assertSame('giveflow', $calls[0]['args'][3]);
    }

    public function test_schedule_recurring_skips_when_already_scheduled(): void
    {
        $GLOBALS['_giveflow_as_has_scheduled'] = true;

        $d = new AsyncDispatcher();
        $d->scheduleRecurring('giveflow.test.hook', 86400);
    ...
    public function test_schedule_recurring_creates_when_not_scheduled(): void
    {
        ...
        $this->assertCount(1, $calls);
        $this->assertSame('giveflow.test.hook', $calls[0]['args'][2]);
        $this->assertSame(86400, $calls[0]['args'][1]);
        $this->assertSame('giveflow', $calls[0]['args'][4]);
    }

    public function test_group_is_always_giveflow(): void
    {
        $d = new AsyncDispatcher();
        $d->enqueue('hook1');
        ...
        foreach ($this->calls() as $call) {
            ...
            $this->assertSame('giveflow', $lastArg, "{$call['func']} group should be 'giveflow'");
        }
    }
}
