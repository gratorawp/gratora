 ...
 * reference prefix GRATORA, an empty legal name. On a screen about retention and
...
function seedApi() {
    apiFetch.mockImplementation( ( { path } ) => {
        if ( path.startsWith( '/gratora/v1/admin/settings/' ) ) {
            return settingsFail
...
const shownPanels = ( root ) =>
    [ ...root.querySelectorAll( '.gratora-settings-page__body > div' ) ].filter( ( d ) => ! d.hidden );
...
test( 'a numbering group that failed to load does not advertise a prefix', async () => {
    ...
    expect( shown ).toContain( 'Could not load these settings.' );
    expect( shown ).not.toContain( 'GRATORA-' );
} );
...
test( 'Retry asks for the group again', async () => {
    ...
    settingsFail = false;
    const before = apiFetch.mock.calls.filter( ( [ a ] ) => a.path === '/gratora/v1/admin/settings/org-profile' ).length;
    retry.click();
    await settle();
    const after = apiFetch.mock.calls.filter( ( [ a ] ) => a.path === '/gratora/v1/admin/settings/org-profile' ).length;
