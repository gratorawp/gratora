...
function page( {
    ...
    const block = `
        <div class="gratora-block gratora-block--donate-button">
            <button type="button" class="gratora-donate-button" data-form-slug="probe"></button>
            <div class="gratora-donate-modal" data-form-slug="probe" hidden>
                <div class="gratora-donate-modal__panel">
                    <button type="button" class="gratora-donate-modal__close" data-gratora-modal-close></button>
                    <div class="gratora-donate-modal__body">
    ...
    return document.querySelector( '.gratora-donate-modal' );
}

// urlReference diverges from reference when the return on the URL is not the
// submission this tab stashed.
function returning( formKey, reference = 'GRATORA-2026-00050', urlReference = reference ) {
    window.history.replaceState( {}, '', '/campaign/?gratora_return=1&gratora_ref=' + urlReference
...
test( 'a donor returning from their bank is shown the modal holding the outcome', async () => {
    ...
    returning( 'gratora-form-1' );

    await loadModalScript();
    await bootRuntime();

    expect( text( 'gratora-form-1' ) ).toContain( THANKS );
...
test( 'the button still opens the modal on a click', async () => {
    ...
    await loadModalScript();
    document.querySelector( '.gratora-donate-button' ).click();

    expect( modal.hidden ).toBe( false );
} );

test( 'a return belonging to an inline form elsewhere on the page does not open the modal', async () => {
    const modal = page( { extraFormId: 'gratora-form-2', hydrate: true } );
    returning( 'gratora-form-2' );
    ...
    // else.
    expect( text( 'gratora-form-2' ) ).toContain( THANKS );
    expect( text( 'gratora-form-1' ) ).not.toContain( THANKS );
    expect( modal.hidden ).toBe( true );
} );
...
test( 'a stash naming no form opens the modal that holds the first form on the page', async () => {
    ...
    expect( text( 'gratora-form-1' ) ).toContain( THANKS );
    expect( text( 'gratora-form-2' ) ).not.toContain( THANKS );
    expect( modal.hidden ).toBe( false );
} );

test( 'the same page with the inline form first leaves the modal shut', async () => {
    const modal = page( { extraFormId: 'gratora-form-2', hydrate: true, extraFirst: true } );
    returning( null );
...
describe( 'the reveal agrees with the form that actually claimed the return', () => {
    ...
    test( 'a return carrying a reference this tab never stashed is left alone', async () => {
        ...
        const modal = page( { hydrate: true } );
        returning( 'gratora-form-1', 'GRATORA-2026-00050', 'GRATORA-2026-00099' );
    ...
    test( 'a modal script that evaluates after the runtime stripped the URL still reveals', async () => {
        ...
        await bootRuntime();
        expect( window.location.search ).not.toContain( 'gratora_return' );
    ...
    test( 'a stashed reference that came back as a number still matches the URL', async () => {
        ...
        const modal = page( { hydrate: true } );
        returning( 'gratora-form-1', 20260050, '20260050' );
    ...
    test( 'a form the shortcode gave no id still opens the modal around it', async () => {
        ...
        expect( document.querySelector( '.gratora-donation-form' ).textContent ).toContain( THANKS );
        expect( modal.hidden ).toBe( false );
    } );
