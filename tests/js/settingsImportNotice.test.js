...
async function choose( payload ) {
    ...
    Object.defineProperty( input, 'files', {
        configurable: true,
        value: [ { name: 'gratora-export.json', text: async () => JSON.stringify( payload ) } ],
    } );
...
test( 'the settings confirm does not promise to clear settings the file omits', async () => {
    ...
    const confirm = await choose( { settings: { gratora_receipt_settings: { header_title: 'Imported' } } } );
...
test( 'a full export is told its settings half is written over the site', async () => {
    ...
    const confirm = await choose( { tables: {}, settings: { gratora_gateway_config: { stripe: {} } } } );
...
test( 'a refused import still reports the settings groups that landed', async () => {
    ...
    apiFetch.mockImplementation( () => Promise.reject( {
        code:    'gratora_base_currency_locked',
        message: 'Part of that file was not restored. The base currency stays EUR.',
        data:    { status: 409, applied: 1, refused: { gratora_currency: 'locked' }, imported: false, records: null },
    } ) );

    const confirm = await choose( { settings: { gratora_currency: {}, gratora_receipt_settings: {} } } );
