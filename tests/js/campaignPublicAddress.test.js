...
test( 'the public URL is the page permalink, not the campaign slug', () => {
    ...
    const link = root.querySelector( '.dono-url-preview a' );
    expect( link.getAttribute( 'href' ) ).toBe( 'https://example.test/campaigns/support-us-2/' );

    const shown = root.querySelector( '.dono-url-preview .url' ).textContent;
...
test( 'a permalink outside the campaigns prefix is shown as it is', () => {
    ...
    expect( root.querySelector( '.dono-url-preview a' ).getAttribute( 'href' ) ).toBe( 'https://example.test/?page_id=41' );
    expect( root.querySelector( '.dono-url-preview .url' ).textContent ).not.toContain( '/campaigns/' );
} );
...
test( 'a campaign with no page advertises no address at all', () => {
    ...
    expect( root.querySelector( '.dono-url-preview' ) ).toBeNull();
