...
const primary = () => document.querySelector( '.dono-form__button--primary' );
const back    = () => document.querySelector( '.dono-form__button--secondary' );
...
const paymentPanel = () => document.querySelector( '.dono-form--payment' );
...
test( 'a block on the submit page is still hosted there, not replaced', async () => {
    ...
    expect( document.querySelector( '.dono-form--payment' ) ).toBeNull();
} );
...
describe( 'a stale nonce does not cost the donation', () => {
    ...
    test( 'the submit is retried once without it, and goes through', async () => {
        ...
        expect( document.querySelector( '.dono-form--payment' ) ).toBeTruthy();
    } );
    ...
    test( 'a 403 that is not about the nonce is not retried', async () => {
        global.fetch = jest.fn( () => Promise.resolve( {
            ...
            status: 403,
            clone() { return this; },
            json:   () => Promise.resolve( { code: 'dono_forbidden', message: 'No.' } ),
        } ) );
