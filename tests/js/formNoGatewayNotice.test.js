...
test( 'a paged form explains itself on the page the submit is on', async () => {
    ...
    expect( document.querySelectorAll( '.dono-form__payment .dono-form__gateways-empty' ) ).toHaveLength( 1 );

    document.querySelector( '.dono-form__button--primary' ).click();
    await settle();

    expect( document.querySelector( '.dono-form__page-title' ).textContent ).toBe( 'Details' );
    expect( notices() ).toEqual( [ NONE ] );
    // And here it is the submit speaking, because the section stayed behind.
    expect( document.querySelectorAll( '.dono-form__payment' ) ).toHaveLength( 0 );
} );
