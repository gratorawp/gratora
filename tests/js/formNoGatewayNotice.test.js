...
const button = () => document.querySelector( '.gratora-form__button--primary' );
...
test( 'a paged form explains itself on the page the submit is on', async () => {
    ...
    expect( document.querySelectorAll( '.gratora-form__payment .gratora-form__gateways-empty' ) ).toHaveLength( 1 );

    document.querySelector( '.gratora-form__button--primary' ).click();
    await settle();

    expect( document.querySelector( '.gratora-form__page-title' ).textContent ).toBe( 'Details' );
    expect( notices() ).toEqual( [ NONE ] );
    // And here it is the submit speaking, because the section stayed behind.
    expect( document.querySelectorAll( '.gratora-form__payment' ) ).toHaveLength( 0 );
} );
