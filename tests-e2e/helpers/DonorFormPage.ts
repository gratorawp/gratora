...
export class DonorFormPage {
    ...
    constructor(page: Page) {
        page.on('response', async (res) => {
            if (this.lastFailure || res.status() < 400 || ! res.url().includes('/gratora/v1/')) {
                return;
            }
        ...
        this.page = page;
        this.form = page.locator('form.gratora-donation-form').first();
    }

    async open(path = process.env.GRATORA_E2E_FORM_PATH ?? '/'): Promise<void> {
        await this.page.goto(path);
        await this.form.waitFor({ state: 'attached' });
        // The runtime sets data-gratora-ready on every mount() exit path; the
        // JS-gated cloak only releases when it's set.
        await expect(this.form).toHaveAttribute('data-gratora-ready', 'true', { timeout: 10_000 });
        this.renderedAt = Date.now();
    }

    presets(): Locator {
        return this.form.locator('.gratora-form__preset');
    }
    ...
    async fillCustomAmount(value: number): Promise<void> {
        const input = this.form.locator('.gratora-form__custom input').first();
        await input.fill(String(value));
    }

    currencySwitcher(): Locator {
        return this.form.locator('.gratora-form__currency-switcher');
    }
    ...
    async selectCurrency(code: string): Promise<void> {
        ...
        await sw.locator('.gratora-form__currency-pill', { hasText: code }).click();
    }
    ...
    countrySelect(root?: Locator): Locator {
        return (root ?? this.form).locator('.gratora-form__country-select').first();
    }
    ...
    async pickCountry(name: string, root?: Locator): Promise<void> {
        ...
        await cs.locator('.gratora-form__country-select-option').filter({ hasText: name }).first().click();
    }

    addressFieldset(): Locator {
        return this.form.locator('.gratora-form__address').first();
    }
    ...
    async fillComment(value: string): Promise<void> {
        ...
        await this.form.locator('.gratora-form__field textarea').first().fill(value);
    }

    anonymousToggle(): Locator {
        // Anonymous block renders as a bare `.gratora-form__check`; cover-fees
        // shares that base class but adds `gratora-form__cover-fees`. The
        // :not() filter rules cover-fees out.
        return this.form
            .locator('.gratora-form__check:not(.gratora-form__cover-fees) input[type="checkbox"]')
            .first();
    }

    consentFieldset(): Locator {
        return this.form.locator('.gratora-form__consent').first();
    }

    coverFeesToggle(): Locator {
        return this.form.locator('.gratora-form__cover-fees input[type="checkbox"], input[name="cover_fees"]').first();
    }

    gatewayOptions(): Locator {
        return this.form.locator('.gratora-form__gateway input[type="radio"]');
    }

    async selectGateway(id: string): Promise<void> {
        const radio = this.form.locator(`.gratora-form__gateway input[type="radio"][value="${id}"]`);
    ...
    async acceptRequiredConsents(): Promise<void> {
        const required = this.consentFieldset()
            .locator('label:has(.gratora-form__consent-required-pill) input[type="checkbox"]');
    ...
    async submit(opts: { consents?: boolean } = {}): Promise<void> {
        ...
        await this.form.locator('.gratora-form__button--primary').click();
    }

    successCard(): Locator {
        return this.form.locator('.gratora-form__success');
    }
    ...
    async expectThankYou(): Promise<void> {
        try {
            ...
            const banner = await this.form.locator('.gratora-form__error').allTextContents();
            // "Required." on its own does not say which field, and every
            // required field says it. Name what it is attached to.
            const fields = await this.form.locator('.gratora-form__field-error')
                .filter({ hasText: /\S/ })
                .evaluateAll((nodes) => nodes.map((n) => {
                    const field = n.closest('label, .gratora-form__field, fieldset');
                    const input = field?.querySelector('input, select, textarea') as HTMLInputElement | null;
                    const named = input?.name || input?.id || field?.querySelector('legend, .gratora-form__label')?.textContent;
    ...
    async expectFieldError(field: string): Promise<void> {
        await expect(this.form.locator('.gratora-form__field-error').filter({ hasText: /\S/ }).first())
