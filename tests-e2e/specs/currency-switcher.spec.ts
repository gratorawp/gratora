...
test.describe('currency-switcher block', () => {
    ...
    test('changing currency updates preset tile amounts', async ({ donor }) => {
        ...
        const before = (await firstPreset.locator('.giveflow-form__preset-amount').textContent()) ?? '';
        ...
        if (await select.count() > 0) {
            ...
            const pills = donor.currencySwitcher().locator('.giveflow-form__currency-pill');
        ...
        const after = (await firstPreset.locator('.giveflow-form__preset-amount').textContent()) ?? '';
