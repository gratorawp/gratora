/**
 * wp-admin screenshot capture. Not an assertion suite: it walks every FundKit
 ...
 * the donation form.
 *
 * Its own opt-in project (FUNDKIT_E2E_SHOTS=1) so a normal e2e run never spends
 * minutes writing images. Output lands in tests-e2e/screenshots (gitignored),
 * or FUNDKIT_E2E_SHOTS_DIR.
 *
 *   npm run test:shots
 *
...
const ROOT = {
    dashboard:     '#fundkit-admin-dashboard > *',
    campaigns:     '#fundkit-admin-campaigns > *',
    donations:     '#fundkit-admin-donations > *',
    donors:        '#fundkit-admin-donors > *',
    subscriptions: '#fundkit-admin-subscriptions > *',
    funds:         '#fundkit-admin-funds > *',
    settings:      '#fundkit-admin-settings > *',
    tools:         '#fundkit-admin-tools > *',
};
...
async function firstCampaignId(page: import('@playwright/test').Page): Promise<string | null> {
    const href = await firstHref(page, '#fundkit-admin-campaigns a[href*="view=detail"]');
...
test.describe('admin screenshots', () => {
    ...
    test('dashboard', async ({ page }) => {
        await openAdmin(page, screen('fundkit'), ROOT.dashboard);
        await shoot(page, 'admin-dashboard');
    });

    test('campaigns', async ({ page }) => {
        await openAdmin(page, screen('fundkit-campaigns'), ROOT.campaigns);
        ...
        for (const tab of ['overview', 'forms', 'settings']) {
            await openAdmin(page, screen('fundkit-campaigns', `&view=detail&id=${id}&tab=${tab}`), ROOT.campaigns);
    ...
    test('form builder', async ({ page }) => {
        ...
        await openAdmin(page, screen('fundkit-campaigns', `&view=detail&id=${id}&tab=forms`), ROOT.campaigns);
        const href = await firstHref(page, 'a[href*="page=fundkit-forms&form="]');
        ...
        await openAdmin(page, screen('fundkit-forms', `&form=${formId}`), '.fundkit-form-editor__canvas');
        ...
        await openTab(page, '.fundkit-editor-header__tab:has-text("Settings")');
        await shoot(page, 'admin-form-builder-settings');
    });

    test('donations', async ({ page }) => {
        await openAdmin(page, screen('fundkit-donations'), ROOT.donations);
        await shoot(page, 'admin-donations-list');

        const href = await firstHref(page, '#fundkit-admin-donations a[href*="view=detail"]');
        ...
        await openAdmin(page, screen('fundkit-donations', `&view=detail&reference=${reference}`), ROOT.donations);
        await shoot(page, 'admin-donation-detail');
    });

    test('donors', async ({ page }) => {
        await openAdmin(page, screen('fundkit-donors'), ROOT.donors);
        await shoot(page, 'admin-donors-list');

        await openAdmin(page, screen('fundkit-donors', '#insights'), ROOT.donors);
        await shoot(page, 'admin-donors-insights');

        await openAdmin(page, screen('fundkit-donors'), ROOT.donors);
        const href = await firstHref(page, '#fundkit-admin-donors a[href^="#donor/"]');
        test.skip(! href, 'no donors on this site; donor profile not captured');

        await openAdmin(page, screen('fundkit-donors', href as string), ROOT.donors);
    ...
    test('subscriptions', async ({ page }) => {
        await openAdmin(page, screen('fundkit-subscriptions'), ROOT.subscriptions);
        await shoot(page, 'admin-subscriptions-list');
    });

    test('funds', async ({ page }) => {
        await openAdmin(page, screen('fundkit-funds'), ROOT.funds);
        await shoot(page, 'admin-funds-list');
    });

    test('settings', async ({ page }) => {
        for (const tab of SETTINGS_TABS) {
            await openAdmin(page, screen('fundkit-settings', `#${tab}`), ROOT.settings);
    ...
    test('tools', async ({ page }) => {
        for (const tab of TOOLS_TABS) {
            await openAdmin(page, screen('fundkit-tools', `#${tab}`), ROOT.tools);
