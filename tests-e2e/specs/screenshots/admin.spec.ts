/**
 * wp-admin screenshot capture. Not an assertion suite: it walks every GiveFlow
 ...
 * the donation form.
 *
 * Its own opt-in project (GIVEFLOW_E2E_SHOTS=1) so a normal e2e run never spends
 * minutes writing images. Output lands in tests-e2e/screenshots (gitignored),
 * or GIVEFLOW_E2E_SHOTS_DIR.
 *
 *   npm run test:shots
 *
...
const ROOT = {
    dashboard:     '#giveflow-admin-dashboard > *',
    campaigns:     '#giveflow-admin-campaigns > *',
    donations:     '#giveflow-admin-donations > *',
    donors:        '#giveflow-admin-donors > *',
    subscriptions: '#giveflow-admin-subscriptions > *',
    funds:         '#giveflow-admin-funds > *',
    settings:      '#giveflow-admin-settings > *',
    tools:         '#giveflow-admin-tools > *',
};
...
async function firstCampaignId(page: import('@playwright/test').Page): Promise<string | null> {
    const href = await firstHref(page, '#giveflow-admin-campaigns a[href*="view=detail"]');
...
test.describe('admin screenshots', () => {
    ...
    test('dashboard', async ({ page }) => {
        await openAdmin(page, screen('giveflow'), ROOT.dashboard);
        await shoot(page, 'admin-dashboard');
    });

    test('campaigns', async ({ page }) => {
        await openAdmin(page, screen('giveflow-campaigns'), ROOT.campaigns);
        ...
        for (const tab of ['overview', 'forms', 'settings']) {
            await openAdmin(page, screen('giveflow-campaigns', `&view=detail&id=${id}&tab=${tab}`), ROOT.campaigns);
    ...
    test('form builder', async ({ page }) => {
        ...
        await openAdmin(page, screen('giveflow-campaigns', `&view=detail&id=${id}&tab=forms`), ROOT.campaigns);
        const href = await firstHref(page, 'a[href*="page=giveflow-forms&form="]');
        ...
        await openAdmin(page, screen('giveflow-forms', `&form=${formId}`), '.giveflow-form-editor__canvas');
        ...
        await openTab(page, '.giveflow-editor-header__tab:has-text("Settings")');
        await shoot(page, 'admin-form-builder-settings');
    });

    test('donations', async ({ page }) => {
        await openAdmin(page, screen('giveflow-donations'), ROOT.donations);
        await shoot(page, 'admin-donations-list');

        const href = await firstHref(page, '#giveflow-admin-donations a[href*="view=detail"]');
        ...
        await openAdmin(page, screen('giveflow-donations', `&view=detail&reference=${reference}`), ROOT.donations);
        await shoot(page, 'admin-donation-detail');
    });

    test('donors', async ({ page }) => {
        await openAdmin(page, screen('giveflow-donors'), ROOT.donors);
        await shoot(page, 'admin-donors-list');

        await openAdmin(page, screen('giveflow-donors', '#insights'), ROOT.donors);
        await shoot(page, 'admin-donors-insights');

        await openAdmin(page, screen('giveflow-donors'), ROOT.donors);
        const href = await firstHref(page, '#giveflow-admin-donors a[href^="#donor/"]');
        test.skip(! href, 'no donors on this site; donor profile not captured');

        await openAdmin(page, screen('giveflow-donors', href as string), ROOT.donors);
    ...
    test('subscriptions', async ({ page }) => {
        await openAdmin(page, screen('giveflow-subscriptions'), ROOT.subscriptions);
        await shoot(page, 'admin-subscriptions-list');
    });

    test('funds', async ({ page }) => {
        await openAdmin(page, screen('giveflow-funds'), ROOT.funds);
        await shoot(page, 'admin-funds-list');
    });

    test('settings', async ({ page }) => {
        for (const tab of SETTINGS_TABS) {
            await openAdmin(page, screen('giveflow-settings', `#${tab}`), ROOT.settings);
    ...
    test('tools', async ({ page }) => {
        for (const tab of TOOLS_TABS) {
            await openAdmin(page, screen('giveflow-tools', `#${tab}`), ROOT.tools);
