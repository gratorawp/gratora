...
test.describe('payment step placement', () => {
    ...
    async function stubStripeIntent(page, amountCents = 2500) {
        await page.route('**/giveflow/v1/donations', async (route) => {
            if (route.request().method() !== 'POST') return route.fallback();
            await route.fulfill({
                status: 201,
                contentType: 'application/json',
                body: JSON.stringify({
                    reference:     'GIVEFLOW-E2E-PLACEMENT',
                    status:        'pending',
    ...
    async function whyBlocked(donor, page): Promise<string> {
        ...
        const raw  = /data-giveflow-form-config[^>]*>([\s\S]*?)<\/script>/.exec(html);
        if (! raw) return blocked = 'no GiveFlow form config on the page under test';
    ...
    async function submitToPayment(donor, page): Promise<boolean> {
        ...
        if (await donor.form.locator('.giveflow-form__payment-mount').count() === 0) return false;
        await expect(donor.form.locator('.giveflow-form--settled')).toHaveCount(1, { timeout: 10_000 });
        return true;
    }
    ...
    test('payment mounts at the gateway block, not over the whole form', async ({ donor, page }) => {
        ...
        await expect(donor.form.locator('.giveflow-form__payment-mount')).toHaveCount(1);
        // The selector it replaces is gone, so the donor is not offered a
        // choice they have already made and can no longer change.
        await expect(donor.form.locator('.giveflow-form__gateways')).toHaveCount(0);
    });
    ...
    test('the pay screen carries nothing but the gateway', async ({ donor, page }) => {
        ...
        await expect(donor.form.locator('.giveflow-form__donor').first()).toBeHidden();
        await expect(donor.form.locator('.giveflow-form__amount').first()).toBeHidden();
        await expect(donor.form.locator('.giveflow-form__payment-mount')).toBeVisible();

        // The recap is a block the author places, so a form may not have one.
        // Present or absent it must not be on screen while paying.
        const recap = donor.form.locator('.giveflow-form__confirm');
    ...
    test('the form settles whichever layout it is', async ({ donor, page }) => {
        ...
        const root = donor.form.locator('.giveflow-form--settled');
        await expect(root).toHaveCount(1);

        const variant = await root.evaluate((el) =>
            [...el.classList].find((c) => c.startsWith('giveflow-form--') && c !== 'giveflow-form--settled') || 'dots',
        );
        // Named so a failure says which shape stopped settling.
        expect(['giveflow-form--inline', 'giveflow-form--paged-bar', 'dots']).toContain(variant);
    });
    ...
    test('nothing about the donation can change once payment has started', async ({ donor, page }) => {
        ...
        await page.evaluate(() => {
            const form = document.querySelector('form.giveflow-donation-form');
            form?.querySelectorAll<HTMLElement>('.giveflow-form__preset').forEach((el) => el.click());
    ...
    test('only the gateway offers a way to pay', async ({ donor, page }) => {
        ...
        await expect(donor.form.locator('.giveflow-form__payment-mount .giveflow-form__nav')).toBeVisible();

        const visibleNavs = await donor.form.locator('.giveflow-form__nav:visible').count();
    ...
    test('the donor can actually read and press Pay', async ({ donor, page }) => {
        ...
        await expect(
            donor.form.locator('.giveflow-form__payment-mount .giveflow-form__button--primary'),
        ).toBeVisible();
        ...
        const faded = await donor.form.locator('.giveflow-form__payment-mount').evaluate((el) => {
            for (let n = el as HTMLElement | null; n; n = n.parentElement) {
                if (n.classList?.contains('giveflow-form')) break;
    ...
    test('nothing outside the gateway can be interacted with', async ({ donor, page }) => {
        ...
        const reachable = await donor.form.evaluate((form) => {
            const mount = form.querySelector('.giveflow-form__payment-mount');
            return [...form.querySelectorAll('input, select, textarea, button')]
                ...
                .filter((el) => ! el.closest('.giveflow-form__hp'))
    ...
    test('the thank-you card says what was donated', async ({ donor, page }) => {
        ...
        const receipt = donor.form.locator('.giveflow-form__summary--receipt');
    ...
    test('the thank-you card offers a way into the portal', async ({ donor, page }) => {
        ...
        await page.route('**/giveflow/v1/portal/send-link', async (route) => {
        ...
        const button = donor.form.locator('.giveflow-form__portal-link');
        ...
        await expect(donor.form.locator('.giveflow-form__portal-link.is-sent')).toBeVisible();
        await expect(donor.form.locator('.giveflow-form__portal-link')).toBeDisabled();
    ...
    test('a gateway with no payment step is unaffected', async ({ donor, page }) => {
        ...
        await donor.expectThankYou();
        await expect(donor.form.locator('.giveflow-form__payment-mount')).toHaveCount(0);
    });
});
