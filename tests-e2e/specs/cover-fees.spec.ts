/**
 * `fundkit/cover-fees` block - donor opts to absorb the processing fee. Toggling
...
test.describe('cover-fees block', () => {
    ...
    test('toggling shows the fee in the label', async ({ donor }) => {
        ...
        const label = donor.form.locator('.fundkit-form__cover-fees').first();
        if (! await toggle.isChecked()) await toggle.click();
        // The fee-math span renders inside an <em>; appears once a fee > 0
        // resolves from the cover-fees percent/fixed config.
        await expect(label.locator('.fundkit-form__cover-fees-math')).toBeVisible();
    });
