 ...
 * `GIVEFLOW_E2E_MULTI_STEP_FORM_PATH`. The seed (`wp giveflow e2e-seed`) creates one
 * with these pages:
 *   step 1: giveflow/donation-amount (presets, required)
 *   step 2: giveflow/name + giveflow/email (both required)
 *   step 3: giveflow/payment-gateways + giveflow/submit-button
 *
...
test.describe('multi-step wizard', () => {
    ...
    test('donor step renders fields after continuing from amount', async ({ donor }) => {
        ...
        await donor.form.locator('.giveflow-form__button--primary').click();
    ...
    test('Next does not advance when the amount is cleared to zero', async ({ donor }) => {
        ...
        const firstDot = donor.form.locator('.giveflow-form__progress-dot').nth(0);
    ...
    test('Back navigation preserves filled values across steps', async ({ donor }) => {
        ...
        // Back to step 1.
        await donor.form.locator('.giveflow-form__button--secondary').click();
        const firstDot = donor.form.locator('.giveflow-form__progress-dot').nth(0);
        await expect(firstDot).toHaveClass(/is-current/);
        // Preset 0 is still the selected one.
        await expect(donor.form.locator('.giveflow-form__preset').nth(0)).toHaveClass(/is-selected/);
    ...
    test('step indicator labels match the seeded step titles', async ({ donor }) => {
        ...
        const dots = donor.form.locator('.giveflow-form__progress-dot');
    ...
    test('a missing required field on step 2 blocks the move to step 3', async ({ donor }) => {
        ...
        await expect(donor.form.locator('.giveflow-form__progress-dot').nth(1)).toHaveClass(/is-current/);
