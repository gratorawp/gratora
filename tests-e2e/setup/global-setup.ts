...
function requireEnv(name: string): string {
    ...
    if (! v || v.trim() === '') {
        throw new Error(
            `Missing env ${name}. Set GIVEFLOW_E2E_URL (e.g. http://localhost:10075) in ` +
 ...
 * (GIVEFLOW_E2E_FORM_PATH), and the variant forms their own paths. Each check runs
...
export default async function globalSetup(_config: FullConfig): Promise<void> {
    const baseURL = requireEnv('GIVEFLOW_E2E_URL');
    ...
    try {
        ...
        const formPath = process.env.GIVEFLOW_E2E_FORM_PATH;
        if (formPath && formPath.trim() !== '') {
            const res = await ctx.get(formPath);
            if (! res.ok()) {
                throw new Error(
                    `Form page not reachable: GET ${baseURL}${formPath} -> ${res.status()}. ` +
                    `Run \`wp giveflow e2e-seed\`. See tests-e2e/README.md.`,
                );
            }
            const html = await res.text();
            if (! /<form[^>]*class="[^"]*giveflow-donation-form/.test(html)) {
                throw new Error(
                    `Page ${formPath} did not render a GiveFlow donation form. ` +
                    `Run \`wp giveflow e2e-seed\` and ensure the form is published.`,
                );
            }
        }

        const condPath = process.env.GIVEFLOW_E2E_CONDITIONAL_FORM_PATH;
        ...
        const customFieldsPath = process.env.GIVEFLOW_E2E_CUSTOM_FIELDS_FORM_PATH;
        ...
        const layoutPath = process.env.GIVEFLOW_E2E_LAYOUT_FORM_PATH;
