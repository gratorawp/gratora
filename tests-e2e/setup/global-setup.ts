...
function requireEnv(name: string): string {
    ...
    if (! v || v.trim() === '') {
        throw new Error(
            `Missing env ${name}. Set FUNDKIT_E2E_URL (e.g. http://localhost:10075) in ` +
 ...
 * (FUNDKIT_E2E_FORM_PATH), and the variant forms their own paths. Each check runs
...
export default async function globalSetup(_config: FullConfig): Promise<void> {
    const baseURL = requireEnv('FUNDKIT_E2E_URL');
    ...
    try {
        ...
        const formPath = process.env.FUNDKIT_E2E_FORM_PATH;
        if (formPath && formPath.trim() !== '') {
            ...
            if (! /<form[^>]*class="[^"]*fundkit-donation-form/.test(html)) {
                throw new Error(
        ...
        const condPath = process.env.FUNDKIT_E2E_CONDITIONAL_FORM_PATH;
        ...
        const customFieldsPath = process.env.FUNDKIT_E2E_CUSTOM_FIELDS_FORM_PATH;
        ...
        const layoutPath = process.env.FUNDKIT_E2E_LAYOUT_FORM_PATH;
