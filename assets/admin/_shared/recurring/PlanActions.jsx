...
export function dueIn( iso ) {
    ...
    if ( days < 0 ) {
        ...
        return sprintf( _n( '%d day overdue', '%d days overdue', Math.abs( days ), 'dono' ), Math.abs( days ) );
    }
    if ( days === 0 ) return __( 'today', 'dono' );
    /* translators: %d: days until the next charge. */
    return sprintf( _n( 'in %d day', 'in %d days', days, 'dono' ), days );
}
...
export function retryActionFor( plan ) {
    ...
    return { id: 'retry', label: __( 'Retry payment', 'dono' ) };
}
...
export function actionsFor( plan ) {
    ...
    if ( plan.status === 'paused' ) {
        actions.push( { id: 'resume', label: __( 'Resume', 'dono' ) } );
    } else {
        actions.push( { id: 'pause', label: __( 'Pause', 'dono' ) } );
        actions.push( { id: 'skip_next', label: __( 'Skip next', 'dono' ) } );
    }
    actions.push( { id: 'change_amount', label: __( 'Change amount', 'dono' ) } );
    actions.push( { id: 'cancel', label: __( 'Cancel', 'dono' ), destructive: true } );

    return actions;
}

const TITLES = {
    retry:         __( 'Retry the payment', 'dono' ),
    pause:         __( 'Pause this donation', 'dono' ),
    resume:        __( 'Resume this donation', 'dono' ),
    skip_next:     __( 'Skip the next payment', 'dono' ),
    change_amount: __( 'Change the amount', 'dono' ),
    cancel:        __( 'Cancel this donation', 'dono' ),
};
    ...
    const submit = () => {
        ...
        if ( action === 'change_amount' ) {
            ...
            if ( ! Number.isFinite( cents ) || cents <= 0 ) {
                setError( __( 'Enter an amount.', 'dono' ) );
                return;
            }
        ...
        apiFetch( { path: `/dono/v1/admin/recurring/${ plan.id }/action`, method: 'POST', data: body } )
            ...
            .catch( ( e ) => setError( e?.message || __( 'That change could not be made.', 'dono' ) ) )
            .finally( () => setBusy( false ) );
    };

    return (
        <Dialog
            title={ TITLES[ action ] || __( 'Change this donation', 'dono' ) }
            ...
            foot={
                <>
                    ...
                    >
                        { busy
                            ? __( 'Working…', 'dono' )
                            : ( action === 'retry' ? __( 'Retry now', 'dono' ) : __( 'Apply change', 'dono' ) ) }
                    </Btn>
                </>
            }
        >
            ...
            { action === 'pause' && (
                <p>
                    <label>
                        <span style={ { display: 'block', marginBottom: 4 } }>{ __( 'Pause for', 'dono' ) }</span>
                        <select
            ...
            { action === 'cancel' && (
                <p>
                    <label>
                        <span style={ { display: 'block', marginBottom: 4 } }>{ __( 'Reason (optional)', 'dono' ) }</span>
                        <input
                            type="text"
                            className="dono-input"
                            value={ reason }
            ...
            { action === 'retry' && (
                <p>
                    { __( 'The gateway will try to collect the outstanding renewal again now. If it succeeds the donation appears within a few moments, once the gateway confirms it.', 'dono' ) }
                </p>
            ) }

            { action === 'skip_next' && (
                <p>{ __( 'The next payment is skipped and the donation carries on one cycle later. Nothing is charged in between.', 'dono' ) }</p>
            ) }

            { action === 'resume' && (
                <p>{ __( 'Charging restarts on the plan’s normal schedule.', 'dono' ) }</p>
            ) }
            ...
            { action !== 'cancel' && action !== 'retry' && (
                <div style={ { marginTop: 12, display: 'flex', alignItems: 'center', gap: 8 } }>
                    <Switch
                        checked={ notify }
                        onChange={ setNotify }
                        label={ __( 'Notify donor', 'dono' ) }
                    />
                    <span>{ __( 'Email the donor about this change', 'dono' ) }</span>
                </div>
            ) }
