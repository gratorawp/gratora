...
export function GoalCell( { item } ) {
    ...
    const template = type === 'donors'
        ? /* translators: %s: number of donors */ _n( '%s donor', '%s donors', target, 'fundkit-fundraising-campaigns' )
        : /* translators: %s: number of donations */ _n( '%s donation', '%s donations', target, 'fundkit-fundraising-campaigns' );
    const label = ! hasGoal
        ? __( 'No goal', 'fundkit-fundraising-campaigns' )
        : isAmount
