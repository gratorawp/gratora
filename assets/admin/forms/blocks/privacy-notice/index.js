...
function readPrivacyUrl() {
    ...
    return String( window.gratora?.privacy_policy_url || '' );
}
...
function Edit( { attributes, setAttributes } ) {
    ...
    const settingsHref = ( typeof window !== 'undefined' && window.gratora?.wp?.settings_url )
        ? `${ window.gratora.wp.settings_url }#privacy`
        : '#';

    const blockProps = useBlockProps( {
        className: 'gratora-block-preview gratora-block-preview--privacy',
                ...
                <p style={ { margin: 0, fontSize: 13, color: '#6b7280', lineHeight: 1.5 } }>
                    ...
                    { hasUrl && (
                        <a
                            ...
                            style={ { color: 'var(--gratora-accent, #211d3f)', textDecoration: 'underline' } }
                        >
                            { linkText }
                        </a>
                    ) }
