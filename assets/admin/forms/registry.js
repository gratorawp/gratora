 ...
 * window.gratora.blocks.register(name, def) after the bundle loads, and the
 * 'gratora.editor.registerBlocks' action fired before mount; both whitelist for the inserter.
 */
...
/**
 * Defaults merged into every Gratora block's `supports` at registration time.
...
const api = {
    ...
    register( name, definition ) {
        if ( ! name || typeof name !== 'string' ) {
            throw new Error( 'gratora.blocks.register: name must be a non-empty string' );
        }
        allowed.add( name );
        const merged = {
            ...definition,
...
export function runBlockRegistration() {
    doAction( 'gratora.editor.registerBlocks', api );
}

// Expose on window.gratora for the imperative path.
if ( typeof window !== 'undefined' ) {
    window.gratora = window.gratora || {};
    window.gratora.blocks = api;
    window.gratora.editorHooks = {
        registerBlocks: 'gratora.editor.registerBlocks',
    };
}
