...
export default function CsvImportCard( { setNotice } ) {
    ...
    const choose = async ( file ) => {
        ...
        try {
            ...
            const res  = await apiFetch( {
                path: '/gratora/v1/admin/tools/csv-inspect',
                method: 'POST',
                data: { csv: text },
            } );
            setCsv( text );
    ...
    const run = async ( dryRun ) => {
        ...
        try {
            const res = await apiFetch( {
                path: '/gratora/v1/admin/tools/csv-import',
                method: 'POST',
            ...
            <tr key={ field }>
                <th scope="row">
                    ...
                    { ( field === 'email' || ( field === 'date' && withAmount ) ) && (
                        <span className="gratora-csv-map__req"> *</span>
                    ) }
                </th>
                <td>
                ...
                <td className="gratora-csv-map__sample">{ sample || '-' }</td>
            </tr>
        );
    };
        ...
        >
            ...
            { inspected && (
                <>
                    ...
                    { preview && (
                        <div className="gratora-csv-preview">
                            <p><strong>{ summarise( preview ) }</strong></p>
                            { Object.keys( preview.skipped || {} ).length > 0 && (
                                <ul className="gratora-csv-preview__skips">
                            ...
                            { ( preview.errors || [] ).length > 0 && (
                                <ul className="gratora-csv-preview__errors">
