 ...
 *
 * GET /gratora/v1/admin/donations/{reference} returns:
...
export default function Detail( { reference } ) {
    ...
    const releaseRefund = ( refund ) => {
        setConfirm( {
            ...
            onConfirm: async () => {
                try {
                    await apiFetch( {
                        path:   `/gratora/v1/admin/donations/${ donation.reference }/release-refund`,
                        method: 'POST',
    ...
    const submitFailed = async () => {
        ...
        try {
            await apiFetch( {
                path:   `/gratora/v1/admin/donations/${ donation.reference }/mark-failed`,
                method: 'POST',
                data:   reason ? { reason } : {},
            } );
            setFailOpen( false );
