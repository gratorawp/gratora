...
export default function RefundDialog( { donation, onClose, onSuccess, plan = null } ) {
    ...
    const submit = async ( e ) => {
        ...
        if ( ! cents || cents <= 0 || cents > maxCents ) {
            setError( sprintf( /* translators: 1: minimum amount, 2: maximum amount */ __( 'Amount must be between %1$s and %2$s', 'dono-fundraising-platform' ), formatAmount( 1, donation.currency ), formatAmount( maxCents, donation.currency ) ) );
            return;
        }
        ...
        try {
            ...
            setError( err?.message || __( 'Refund failed', 'dono-fundraising-platform' ) );
        } finally {
            setSaving( false );
        }
    };
    ...
    return (
        <Modal
            title={ __( 'Issue refund', 'dono-fundraising-platform' ) }
        ...
        >
            <form onSubmit={ submit } className="dd-edit-form">
                ...
                <label>
                    { __( 'Reason', 'dono-fundraising-platform' ) }
                    <input
                        className="dono-input"
                        type="text"
                        value={ reason }
                        onChange={ ( e ) => setReason( e.target.value ) }
                        placeholder={ __( 'optional', 'dono-fundraising-platform' ) }
                        maxLength={ 200 }
                    />
                </label>
                { planLive && (
                    <>
                        <div className="dd-banner dd-banner--warn" style={ { gridColumn: '1 / -1' } } role="status">
                            ...
                            <div className="dd-banner__body">
                                <strong>{ __( 'The recurring schedule is still running.', 'dono-fundraising-platform' ) }</strong>{ ' ' }
                                { plan.next_payment_at
                                    ? sprintf(
                                        /* translators: %s: date of the next scheduled payment */
                                        __( 'Refunding this donation does not stop it, and the donor will be charged again on %s.', 'dono-fundraising-platform' ),
                                        formatDate( plan.next_payment_at )
                                    )
                                    : __( 'Refunding this donation does not stop it, and the donor will be charged again.', 'dono-fundraising-platform' ) }
                            </div>
                        </div>
                        <label style={ { gridColumn: '1 / -1', flexDirection: 'row', alignItems: 'flex-start', gap: 8 } }>
                            ...
                            />
                            <span>
                                { __( 'Cancel the recurring schedule as well', 'dono-fundraising-platform' ) }
                                <span style={ { display: 'block', marginTop: 2, fontSize: 12.5 } }>
                                    { __( 'The donor is emailed when a schedule is cancelled.', 'dono-fundraising-platform' ) }
                                </span>
                            </span>
                        </label>
                    </>
                ...
                <div className="dd-edit-form__actions">
                    ...
                    <button type="submit" className="btn btn--danger" disabled={ saving }>
                        { saving ? __( 'Refunding…', 'dono-fundraising-platform' ) : __( 'Issue refund', 'dono-fundraising-platform' ) }
                    </button>
                </div>
            </form>
        </Modal>
