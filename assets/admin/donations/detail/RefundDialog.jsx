...
export default function RefundDialog( { donation, onClose, onSuccess, plan = null } ) {
    ...
    const submit = async ( e ) => {
        ...
        if ( ! cents || cents <= 0 || cents > maxCents ) {
            setError( sprintf( /* translators: 1: minimum amount, 2: maximum amount */ __( 'Amount must be between %1$s and %2$s', 'gratora-donation-platform' ), formatAmount( 1, donation.currency ), formatAmount( maxCents, donation.currency ) ) );
            return;
        }
        ...
        try {
            ...
            setError( err?.message || __( 'Refund failed', 'gratora-donation-platform' ) );
        } finally {
            setSaving( false );
        }
    };
    ...
    return (
        <Modal
            title={ __( 'Issue refund', 'gratora-donation-platform' ) }
        ...
        >
            <form onSubmit={ submit } className="dd-edit-form">
                <p style={ { gridColumn: '1 / -1', color: 'var(--dd-text-muted, #6b7280)', fontSize: 13, marginTop: 0 } }>
                    { sprintf(
                        /* translators: %s: amount */ __( 'Up to %s can be refunded back to the donor. Stripe refunds typically settle in 5-10 business days.', 'gratora-donation-platform' ),
                        formatAmount( maxCents, donation.currency )
                    ) }
                    { pendingCents > 0 && ' ' + sprintf(
                        /* translators: %s: amount already sent to the gateway */
                        __( '%s is already on its way back to the donor and has not settled yet, so it is not offered here.', 'gratora-donation-platform' ),
                ...
                <label>
                    { __( 'Reason', 'gratora-donation-platform' ) }
                    <input
                        className="gratora-input"
                        type="text"
                        value={ reason }
                        onChange={ ( e ) => setReason( e.target.value ) }
                        placeholder={ __( 'optional', 'gratora-donation-platform' ) }
                        maxLength={ 200 }
                    />
                </label>
                { planLive && (
                    <>
                        <div className="dd-banner dd-banner--warn" style={ { gridColumn: '1 / -1' } } role="status">
                            ...
                            <div className="dd-banner__body">
                                <strong>{ __( 'The recurring schedule is still running.', 'gratora-donation-platform' ) }</strong>{ ' ' }
                                { plan.next_payment_at
                                    ? sprintf(
                                        /* translators: %s: date of the next scheduled payment */
                                        __( 'Refunding this donation does not stop it, and the donor will be charged again on %s.', 'gratora-donation-platform' ),
                                        formatDate( plan.next_payment_at )
                                    )
                                    : __( 'Refunding this donation does not stop it, and the donor will be charged again.', 'gratora-donation-platform' ) }
                            </div>
                        </div>
                        <label style={ { gridColumn: '1 / -1', flexDirection: 'row', alignItems: 'flex-start', gap: 8 } }>
                            ...
                            />
                            <span>
                                { __( 'Cancel the recurring schedule as well', 'gratora-donation-platform' ) }
                                <span style={ { display: 'block', marginTop: 2, fontSize: 12.5 } }>
                                    { __( 'The donor is emailed when a schedule is cancelled.', 'gratora-donation-platform' ) }
                                </span>
                            </span>
                        </label>
                    </>
                ...
                <div className="dd-edit-form__actions">
                    ...
                    <button type="submit" className="btn btn--danger" disabled={ saving }>
                        { saving ? __( 'Refunding…', 'gratora-donation-platform' ) : __( 'Issue refund', 'gratora-donation-platform' ) }
                    </button>
                </div>
            </form>
        </Modal>
