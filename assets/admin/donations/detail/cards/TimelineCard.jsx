...
function buildEvents( { donation, receipts, refunds, notes } ) {
    ...
    if ( donation.created_at ) {
        events.push( {
            ...
            dot:   'is-info',
            title: __( 'Donation created', 'giveflow-fundraising-campaigns' ),
            sub:   donation.form?.title
                ? <>{ __( 'Through', 'giveflow-fundraising-campaigns' ) } <strong>{ donation.form.title }</strong></>
                : null,
        } );
    }
    if ( donation.paid_at ) {
        events.push( {
            ...
            dot:   'is-ok',
            title: __( 'Payment captured', 'giveflow-fundraising-campaigns' ),
    ...
    if ( donation.status === 'failed' ) {
        ...
        events.push( {
            ...
            dot:   'is-error',
            title: __( 'Marked as failed', 'giveflow-fundraising-campaigns' ),
    ...
    ( receipts || [] ).forEach( ( r, ri ) => {
        events.push( {
            ...
            time:  r.issued_at,
            dot:   'is-info',
            title: __( 'Receipt issued', 'giveflow-fundraising-campaigns' ),
            sub:   <><span className="mono">{ r.receipt_number }</span>{ r.sent_to_email_at && <> · { __( 'emailed', 'giveflow-fundraising-campaigns' ) } { formatDateTime( r.sent_to_email_at ) }</> }</>,
        } );
        if ( r.voided && r.voided_at ) {
            events.push( {
                ...
                time:  r.voided_at,
                dot:   'is-muted',
                title: __( 'Receipt voided', 'giveflow-fundraising-campaigns' ),
    ...
    ( refunds || [] ).forEach( ( r, ri ) => {
        events.push( {
            ...
            title: <>{ __( 'Refund', 'giveflow-fundraising-campaigns' ) } <strong>{ formatAmount( r.amount_cents, r.currency ) }</strong></>,
    ...
    ( notes || [] ).forEach( ( n, ni ) => {
        events.push( {
            ...
            time:  n.created_at,
            dot:   'is-muted',
            title: __( 'Note added', 'giveflow-fundraising-campaigns' ),
